  ...
  const toolPolicyBody = (rows: { server: string; tool: string }[]) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {/* 비영속 제약(스펙 281, 사용자 질문) — 승인 대기는 DB 기록·재개가 필요해 비영속은 승인
            필요 도구를 실행하지 못함(chat.py:1290 fail-closed). 숨기지 않는 이유: '승인 없음' 완화가
            승인 기본 도구를 비영속에서 쓸 유일한 레버. 문구는 긍정형(사용자 원칙: 부정의 부정 제거). */}
        {form.ephemeral && (
          // 한 절·긍정형(사용자 재검토): "(관리자 저장)"은 위 상설 안내가 이미 말하고, "이 도구도"는
          // 지시 대상이 모호했다. 바로 아래 선택지 라벨('승인 없음')에 접지 — '기본값 사용'이 승인
          // 없음인 도구와 오버라이드로 낮춘 도구를 "상태"가 모두 포괄한다.
          <span style={{ fontSize: 12, color: 'var(--color-warning)' }}>
            비영속(1회성)에서는 '승인 없음' 상태의 도구만 실행할 수 있습니다.
          </span>
        )}
