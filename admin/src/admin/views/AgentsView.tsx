...
export default function AgentsView({ onOpenPlayground, meId }: { onOpenPlayground?: (agentRowId: string) => void; meId?: string }) {
  ...
  const save = async (data: AgentFormData) => {
    const config: AgentConfig = {
      ...
      ...(Object.keys(data.modelParams || {}).length ? { modelParams: data.modelParams } : {}),
        ...
        <Popover
          ...
          content={
            <div style={{ fontSize: 13, maxWidth: 320 }}>
              ...
              {/* 푸터 안내는 전부 삭제(사용자 지시) — 출처=탭 이름이 항상 보이고, A2A 제약은
                  상세 A2A 행이 자리에서 설명. 범례는 기호 목록만. */}
            </div>
          }
        >
          ...
          editing
            ? (() => {
                ...
                return {
                  ...
                  modelParams: { ...((c.modelParams as Record<string, boolean>) || a.modelParams || {}) },
                  memories: [...(c.memories || [])],
