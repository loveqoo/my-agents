...
export interface AgentConfig {
  ...
  modelParams?: Record<string, boolean | number> // 모델 설정 오버라이드(스펙 408·411) — enable_thinking·stream·temperature 등
  memories?: string[]
  historyDepth?: number
...
export interface PipelineNode {
  ...
  modelParams?: Record<string, boolean | number> // 노드별 모델 설정 오버라이드(스펙 409·411) — 에이전트 위에 덮음, 미명시=상속
