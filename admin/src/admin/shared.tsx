...
export function DataTable<T>({
  ...
  rowKey?: string
  empty?: ReactNode
  /* 행 강조(스펙 284) — 'green'=내 것(공개), 'blue'=내 것(비공개). 데탑=본행+보조행(tr 클래스),
     모바일=카드 배경. */
  rowTint?: (row: T) => 'green' | 'blue' | undefined
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {rows.length === 0 ? (
          ...
          rows.map((r) => (
            <Panel key={String(cell(r, rowKey))} style={{ padding: 14, ...(rowTint?.(r) === 'green' ? { background: 'rgba(82,196,26,0.09)' } : rowTint?.(r) === 'blue' ? { background: 'rgba(22,119,255,0.07)' } : {}) }}>
              <div
