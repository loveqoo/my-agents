...
async def _make_agent() -> Agent:
    async with SessionLocal() as s:
        existing = (await s.execute(select(Agent).where(Agent.agent_id == AGT))).scalars().first()
...
async def _session_row(sid: str):
    async with SessionLocal() as s:
        return (await s.execute(select(Session).where(Session.session_id == sid))).scalars().first()
...
async def _scoped_targets(agent_pk, days, min_turns) -> set[str]:
    ...
    async with SessionLocal() as s:
        ...
        rows = (
            (
                await s.execute(
                    select(Session.session_id).where(Session.agent_pk == agent_pk, or_(*clauses))
                )
            )
            .scalars()
            .all()
...
async def _global_extra(agent_pk, days, min_turns) -> set[str]:
    ...
    async with SessionLocal() as s:
        ...
        if not clauses:
            return set()
        rows = (
            await s.execute(select(Session.id).where(or_(*clauses), Session.agent_pk != agent_pk))
        ).all()
...
async def section_a(agent) -> None:
    ...
    check(ctx.session_pk is None, "[A1] _load_context: session_pk=None(미영속 보류)")
    check(isinstance(ctx.session_pending, dict), "[A1] session_pending 세팅됨")
    check(str(ctx.session_id or "").startswith("sess-"), "[A1] session_id 즉시 생성(클라 참조용)")
    ...
    check(
        row2 is not None and row2.turns == 2,
        f"[A3] 재호출 turns=2 (실제 {row2.turns if row2 else None})",
    )
    ...
    check(ctx_re.session_pk is not None, "[A4] 기존 session_id 재진입 → session_pk 해석")
    check(ctx_re.session_pending is None, "[A4] 재진입 시 pending None")

    # A5: 승인 게이트 = 실 턴 → _create_approval이 행 보장(resume 연속성).
    ctx_ap = await _load_context(agent.id, None)
    check(ctx_ap.session_pk is None, "[A5] 승인 전 ctx도 미영속 보류")
    apid = await _create_approval(
        ctx_ap, "thread-v049", {"permission": "p", "action": "a", "summary": "s", "args": {}}, None
    )
    ...
    check(ctx_ap.session_pk is not None, "[A5] ctx에 session_pk 기록(연속성)")
    async with SessionLocal() as s:
        ap = (
        ...
        check(
            ap is not None and ap.session_id == ap_sid,
            "[A5] Approval.session_id가 같은 세션 가리킴",
        )
    # resume 시뮬: 같은 session_id로 _load_context → 행을 찾아 새 id 안 만듦.
    ctx_resume = await _load_context(agent.id, ap_sid)
    check(ctx_resume.session_pk is not None, "[A5] resume _load_context가 원 세션 재발견(연속성)")
...
async def _adaptive_real_or_dry(agent, days, min_turns, gone: set[str], kept: set[str], tag: str):
    ...
    if not extra:
        ...
        print(
            f"  note  [{tag}] 라이브 비-fixture 매치 {len(extra)}건 → 실삭제 건너뜀(데이터 보호), dry-run로 검증"
        )
        res = await cleanup_sessions(dry_run=True)
        check(res.get("status") == "dry_run", f"[{tag}] dry-run status=dry_run")
        check(
            res.get("would_delete", 0) >= len(gone),
            f"[{tag}] would_delete가 대상 포함(>= {len(gone)})",
        )
...
async def section_b(agent) -> None:
    ...
    await _seed_session(
        agent.id, f"{SP}t_active", turns=2, age=timedelta(minutes=5)
    )  # idle-guard 보호
    ...
    check(
        scoped == {f"{SP}t_idle"},
        f"[B1] 스코프드 술어: idle만 대상(active/big 보호) — 실제 {scoped}",
    )
    ...
    check(
        res_u.get("retention_days") == 30 and res_u.get("min_session_turns") == 3,
    ...
    await _create_approval(
        ctx_g,
        "thread-v049-guard",
        {"permission": "p", "action": "a", "summary": "s", "args": {}},
        None,
    )
