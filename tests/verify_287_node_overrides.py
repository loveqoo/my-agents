...
def unit_checks() -> None:
    ...
    m, s = _merge_node_overrides(saved, [{"name": "HACK", "prompt": "P1", "model": "m9", "evil": 1}, {"tools": ["y"]}])
    ...
    check(t is not None and t.get("nodes") == {"count": 1, "status": "applied"}, f"U3 nodes 요약 (got {t})")
    ...
    check(t3 is not None and t3.get("nodes", {}).get("status") == "mismatch", "U3 mismatch도 기록(왜 안 먹었는지 표면화)")
...
async def http_checks() -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://t", headers=auth, timeout=120) as c:
        ...
        check(len(base_calls) > 0, f"H1 기준선: echo 호출 실측 (mcp={[(x.get('server'), x.get('tool')) for x in (tr0 or {}).get('mcp', [])]})")
        check("overrides" not in (tr0 or {}), "H1 오버라이드 없음 → trace.overrides 미기록")

        # H2 효과 실측 — 노드 도구를 빼는 오버라이드(같은 구조·필드만) → echo 호출 소멸 + applied.
        ov = {"nodes": [{"prompt": "요청을 처리해라", "model": "mock-llm", "tools": []}], "mcps": [], "vectorTables": [], "memories": []}
        _, tr1 = await _chat(c, pid, "echo 테스트를 해줘", overrides=ov)
        ov_calls = [x for x in (tr1 or {}).get("mcp", []) if x.get("tool", "").endswith("echo")]
        check(len(ov_calls) == 0, f"H2 노드 도구 제거 오버라이드 → echo 호출 0 (got {len(ov_calls)})")
        check((tr1 or {}).get("overrides", {}).get("nodes") == {"count": 1, "status": "applied"},
        ...
        check((tr4 or {}).get("overrides", {}).get("nodes", {}).get("status") == "mismatch",
