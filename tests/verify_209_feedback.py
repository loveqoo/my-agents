    ...
    async with SessionLocal() as s:
        return (
            await s.execute(
                ...
                .where(MessageFeedback.message_pk == asst_mid)
            )
        ).scalar_one()
...
async def main() -> None:
    ...
    try:
        ...
        try:
            ...
            check(
                r.status_code == 200 and r.json()["rating"] == "up",
                f"H1: 소유자 👍 set → 200 up — got {r.status_code}/{r.text[:80]}",
            )
            ...
            check(
                asst is not None
                and asst.get("feedback", {})
                and asst["feedback"]["rating"] == "up",
            ...
            check(
                r.status_code == 200
                and r.json()["rating"] == "down"
                and r.json()["reason"] == "사실 오류",
            ...
            r = await member.put(
                f"/sessions/{own['sid']}/messages/{own['user_mid']}/feedback", json={"rating": "up"}
            )
            check(
                r.status_code == 404,
                f"B1: user 메시지 피드백 → 404(assistant만) — got {r.status_code}",
            )

            # ---- B2 부재 메시지 → 404 ----
            r = await member.put(
                f"/sessions/{own['sid']}/messages/00000000-0000-0000-0000-000000000000/feedback",
                json={"rating": "up"},
            )
            ...
            check(
                r.status_code == 404,
                f"O1(T): member 타인 세션 피드백 → 404(은폐) — got {r.status_code}",
            )
            check(
                await _fb_count(others["asst_mid"]) == 0,
                "O1(T): 타인 메시지 피드백 실제 미생성(주입 차단)",
            )
            ...
            check(
                r.status_code == 403,
                f"O2: 머신 토큰 피드백 → 403(로그인 사용자만) — got {r.status_code}",
            )
            ...
            check(
                r.status_code == 200,
                f"O3: super(admin) 임의 세션 피드백 → 200 — got {r.status_code}",
            )
            check(
                await _fb_count(own["asst_mid"]) == 2, "O3: member+super 각자 1건(owner_id별 분리)"
            )
    ...
    if _fails:
        ...
        sys.exit(1)
    print(
        "✅ 스펙 209 라이브 통합 — 피드백 upsert·소유권 404 은폐·assistant-only·머신 403·DELETE 전부 통과"
    )
