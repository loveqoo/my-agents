    ...
    try:
        ...
        async with httpx.AsyncClient(transport=transport, base_url=VBASE, timeout=120) as c:
            code, raw = await _post_stream(
                c, f"/agents/{pe_id}/a2a", _stream_body("A2A 스트림 스모크"), headers
            )
            reply, final_seen, done_seen, err = _parse_sse(raw)
            check(
                code == 200 and len(reply) > 0 and err is None,
                f"V1b ui message/stream 텍스트 수신 — {len(reply)}자, err={err}",
            )
            check(
                final_seen and done_seen,
                f"V1c final+[DONE] 종결 (final={final_seen}, done={done_seen})",
            )

            # ---- V4 미노출 에이전트 게이트(먼저: pe만 노출, 임의 UUID는 404) ----
            code, _ = await _post_stream(
                c, f"/agents/{_uuid.uuid4()}/a2a", _stream_body("x"), headers
            )
        ...
        async with httpx.AsyncClient(transport=transport, base_url=VBASE, timeout=120) as c:
            code, raw = await _post_stream(
                c, f"/agents/{made[-1]}/a2a", _stream_body("중계 스트림"), headers
            )
            reply, final_seen, done_seen, err = _parse_sse(raw)
            check(
                code == 200 and len(reply) > 0 and err is None,
            ...
            check(err and err.get("code") == -32000, f"V3 중계 루프 거부 -32000 (got {err})")
    finally:
        async with async_session() as s:
