...
async def main():
    ...
    check(
        slugify_name("옵시디언 매니저") == "unnamed",
        "V1f slugify 한글 전탈락→영문 fallback(스펙 217)",
    )
    ...
    try:
        ...
        async with async_session() as s:
            try:
                await AG.create_agent(
                    AgentCreate(name=f"{tag} bad name", config=AgentConfig()),
                    session=s,
                    principal=admin,
                )
        ...
        async with async_session() as s:
            out = await AG.create_agent(
                AgentCreate(
                    name=f"{tag}-agent", description="검증용 에이전트", config=AgentConfig()
                ),
                session=s,
                principal=admin,
            )
            ...
            check(
                out.name == f"{tag}-agent" and out.description == "검증용 에이전트",
                "V2b 정상 생성 + 별명 저장",
            )
        async with async_session() as s:
            try:
                await AG.create_agent(
                    AgentCreate(name=f"{tag}-agent", config=AgentConfig()),
                    session=s,
                    principal=admin,
                )
        ...
        async with async_session() as s:
            try:
                await AG.update_agent(
                    made["agents"][0],
                    AgentUpdate(name="Bad Rename", config=AgentConfig()),
                    session=s,
                    principal=admin,
                )
        ...
        async with async_session() as s:
            out = await AG.update_agent(
                made["agents"][0],
                AgentUpdate(name=None, description="", config=AgentConfig()),
                session=s,
                principal=admin,
            )
        ...
        async with async_session() as s:
            out = await AG.update_agent(
                made["agents"][0],
                AgentUpdate(name=None, description="원본 별명", config=AgentConfig()),
                session=s,
                principal=admin,
            )
        async with async_session() as s:
            ...
            check(
                NAME_RE.match(clone.name) is not None
                and clone.name.startswith(f"{tag}-agent-copy"),
                f"V3a 복제 식별 이름 규칙 준수(스펙 217: 영문 '-copy' 접미): {clone.name!r}",
            )
            check(
                clone.description == "원본 별명 (복사본)", f"V3b 복제 별명: {clone.description!r}"
            )
        async with async_session() as s:
        ...
        async with async_session() as s:
            try:
                await RG.create_collection(
                    CollectionIn(name="Bad Col", embedding_model_id=_uuid.uuid4()),
                    session=s,
                    principal=admin,
                )
                ...
                check(
                    e.status_code == 400 and "규칙" in str(e.detail),
                    f"V5a 컬렉션 위반 400 (got {e.status_code})",
                )
        async with async_session() as s:
            try:
        ...
        async with async_session() as s:
            p = await BL.create_prompt(
                PromptIn(name=f"{tag}-prompt", description="검증 프롬프트"), session=s
            )
            made["prompts"].append(p.id)
            check(
                p.name == f"{tag}-prompt" and p.description == "검증 프롬프트",
                "V5c 프롬프트 정상+별명",
            )
        async with async_session() as s:
            try:
        ...
        async with async_session() as s:
            ...
            p2 = await BL.update_prompt(
                made["prompts"][0], PromptIn(name=f"{tag}-prompt", body="b2"), session=s
            )
        ...
        async with async_session() as s:
            m = await BL.create_mcp_server(
                McpServerIn(name=f"{tag}-mcp", description="검증 MCP"), session=s, principal=admin
            )
        ...
        async with async_session() as s:
            ref_agent = Agent(
                ...
                config={
                    ...
                    "mcps": [],
                    "vectorTables": [],
                    "capabilities": [f"mcp:{tag}-mcp/some-tool"],
                },
            )
            s.add(ref_agent)
            await s.commit()
        ...
        async with async_session() as s:
            try:
                await BL.update_prompt(
                    made["prompts"][0], PromptIn(name=f"{tag}-prompt2"), session=s
                )
                check(False, "V7a 참조 중 프롬프트 rename → 409이어야")
            except HTTPException as e:
                check(
                    e.status_code == 409, f"V7a 참조 중 프롬프트 rename 409 (got {e.status_code})"
                )
        async with async_session() as s:
            try:
        ...
        async with async_session() as s:
            ...
            try:
                await BL.update_mcp_server(
                    made["mcp"][0], McpServerIn(name=f"{tag}-mcp2"), session=s, principal=admin
                )
                check(False, "V7e capabilities 참조 MCP rename → 409이어야")
            except HTTPException as e:
                check(
                    e.status_code == 409,
                    f"V7e capabilities 참조 MCP rename 409 (got {e.status_code})",
                )
