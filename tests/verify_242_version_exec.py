...
async def run() -> bool:
    ...
    async with httpx.AsyncClient(
        ...
        try:
            ...
            sysmsg = next(
                (m["content"] for m in tr.get("sentMessages") or [] if m.get("role") == "system"),
                "",
            )
            ck("V2-PROMPT-MARK" in sysmsg, f"V1a 초안 config로 실행(system=초안 프롬프트)")
            ck(
                tr.get("agentVersion") == v2 and tr.get("versionPinned") is True,
                f"V1b trace 버전 기록 (agentVersion={tr.get('agentVersion')} pinned={tr.get('versionPinned')})",
            )

            # V2 — 무지정: 활성 버전 실행·기록
            r = await c.post(
                f"/agents/{aid}/chat", json={"messages": [{"role": "user", "content": "안녕"}]}
            )
            tr = _last_trace(r.text)
            sysmsg = next(
                (m["content"] for m in tr.get("sentMessages") or [] if m.get("role") == "system"),
            ...
            ck(
                "V1-PROMPT-MARK" in sysmsg
                and tr.get("agentVersion") == v1
                and not tr.get("versionPinned"),
            ...
            r = await c.post(
                f"/agents/{aid}/chat",
                json={"messages": [{"role": "user", "content": "x"}], "version": "v99"},
            )
            ...
            ds = (
                await c.post(
                    "/eval/datasets",
                    json={"name": f"ds242-{uuid.uuid4().hex[:6]}", "kind": "agent"},
                )
            ).json()
            ds_id = ds["id"]
            await c.post(
                f"/eval/datasets/{ds_id}/cases",
                json={"name": "q", "input": "안녕", "asserts": [{"type": "no_error"}]},
            )
            run_ = (
                await c.post(
                    f"/eval/datasets/{ds_id}/runs", json={"agent_id": aid, "agent_version": v2}
                )
            ).json()
            for _ in range(60):
            ...
            env = rr.get("env") or {}
            ck(
                rr.get("status") == "ok" and rr.get("agent_version") == v2,
            ...
            async with SessionLocal() as db:
                before = int(
                    (await db.execute(text("SELECT count(*) FROM approvals"))).scalar_one()
                )
            r = await c.post(
                f"/agents/{aid}/chat",
                json={
                    "messages": [{"role": "user", "content": "레코드 rec-001 삭제해줘"}],
                    "version": v2,
                },
            )
            async with SessionLocal() as db:
            ...
            ck(
                "승인이 필요한 도구를 사용할 수 없습니다" in r.text and after == before,
            ...
            try:
                ...
                ck(
                    r.status_code == 200,
                    f"V6a 공유 에이전트: 멤버 일반 채팅 가능 (got {r.status_code})",
                )
                r = await c.post(
                    f"/agents/{aid}/chat",
                    json={
                ...
                ck(
                    r.status_code == 403,
                    f"V6b 멤버 버전 지정 채팅 403(초안 누출 차단) (got {r.status_code})",
                )
            finally:
                app.dependency_overrides[current_principal] = lambda: _Super()
        finally:
