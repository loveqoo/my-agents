...
async def main():
    ...
    try:
        ...
        if row is None:
            async with async_session() as s:
                r = McpServer(
                    ...
                    transport="http",
                    url=served_url(NAME),
                    tools=list(SERVED_MCP_TOOLS[NAME]),
                    enabled_tools=list(SERVED_MCP_TOOLS[NAME]),
                    status="connected",
                    published=False,
                    owner_id=None,
                )
        ...
        async with async_session() as s:
            try:
                await BL.publish_mcp_server(
                    calc_id, McpPublishIn(published=True), session=s, principal=member
                )
                check(False, "V2a 비소유 member 공개 → 404-fold이어야")
            except HTTPException as e:
                check(
                    e.status_code == 404, f"V2a 비소유 member 공개 404-fold (got {e.status_code})"
                )
        async with async_session() as s:
            out = await BL.publish_mcp_server(
                calc_id, McpPublishIn(published=True), session=s, principal=admin
            )
        ...
        async with async_session() as s:
            await BL.publish_mcp_server(
                calc_id, McpPublishIn(published=False), session=s, principal=admin
            )
        try:
        ...
        async with async_session() as s:
            ext = McpServer(
                name=f"v156-ext-{_uuid.uuid4().hex[:6]}",
                source="external",
                transport="http",
        ...
        async with async_session() as s:
            try:
                await BL.publish_mcp_server(
                    ext_id, McpPublishIn(published=True), session=s, principal=admin
                )
        ...
        async with async_session() as s:
            try:
                await BL.create_mcp_server(
                    McpServerIn(
                        name=f"v156-hijack-{_uuid.uuid4().hex[:6]}",
                        source="custom",
                        transport="http",
                        tools=["add"],
                        enabled_tools=["add"],
                ...
                check(
                    e.status_code == 400, f"V6 source=custom 사용자 생성 400 (got {e.status_code})"
                )
        ...
        async with async_session() as s:
            await BL.publish_mcp_server(
                calc_id, McpPublishIn(published=bool(start_pub)), session=s, principal=admin
            )
    finally:
        async with async_session() as s:
            for kind, val in made:
                if kind == "id":
                    ...
                    obj = (
                        await s.execute(select(McpServer).where(McpServer.name == val))
                    ).scalar_one_or_none()
                if obj is not None:
                    await s.delete(obj)
            await s.commit()
