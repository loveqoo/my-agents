...
def _pass_dims_flag(emb_config: dict) -> bool:
    ...
    cfg = BaseEmbedderConfig(
        model=c["model"], api_key=c.get("api_key"),
        openai_base_url=c.get("openai_base_url"), embedding_dims=c.get("embedding_dims"),
    )
...
FAKE = {
    ...
    "embedder": {"model_id": "snowflake-arctic-embed-l-v2.0", "base_url": "http://x/v1", "api_key": "k"},
}
...
async def main():
    ...
    check("embedding_dims" not in c1["embedder"]["config"], "V1a 네이티브==컬럼: embedding_dims 미포함")
    check(_pass_dims_flag(c1["embedder"]) is False, "V1b 네이티브==컬럼: dimensions 미전송(snowflake 안전)")

    # ---- V2 네이티브≠컬럼(1536) → 컬럼 길이로 전송 ----
    c2 = _build_with_native(1536)
    check(c2["embedder"]["config"].get("embedding_dims") == col,
          f"V2a 네이티브≠컬럼: embedding_dims={col} 전송(절단 의도) (got {c2['embedder']['config'].get('embedding_dims')})")
    check(_pass_dims_flag(c2["embedder"]) is True, "V2b 네이티브≠컬럼: dimensions 전송(text-embedding-3 회귀 방지)")

    # ---- V3 opt-in env override(int 상수) ----
    c3 = _build_with_native(col, request_env=256)
    check(c3["embedder"]["config"].get("embedding_dims") == 256,
    ...
    # ---- V6 컬럼 차원 불변 ----
    check(c1["vector_store"]["config"]["embedding_model_dims"] == col,
    ...
    os.environ["_V159_T"] = "abc"; check(pi("_V159_T") is None, "V7a 비정수 env → None(ValueError 방지)")
    os.environ["_V159_T"] = "0"; check(pi("_V159_T") is None, "V7b '0' → None(dimensions=0 방지)")
    os.environ["_V159_T"] = "-5"; check(pi("_V159_T") is None, "V7c 음수 → None")
    os.environ["_V159_T"] = "512"; check(pi("_V159_T") == 512, "V7d 양수 → int")
    os.environ.pop("_V159_T", None); check(pi("_V159_T") is None, "V7e 미설정 → None")
    ...
    check(mem0_backend._native_embed_dims({"base_url": base, "model_id": mid, "api_key": "keyA"}) == 4242,
          "V8a keyA 캐시 히트")
    # keyB(다른 api_key)는 캐시 미스 → 도달 불가 URL probe 실패 → None(4242 아님 = 키 분리 증명)
    check(mem0_backend._native_embed_dims({"base_url": base, "model_id": mid, "api_key": "keyB"}) != 4242,
    ...
    if backend is None or type(backend).__name__ != "Mem0Backend":
        ...
        try:
            backend.add(scope, [{"role": "user", "content": "The Agent ABCD framework note"}], infer=False)
            await asyncio.sleep(1)
            hits = backend.search(scope, "The Agent ABCD framework note", 4, threshold=0.0)
            check(len(hits) >= 1, f"V5 실 probe로 add+search 왕복 동작(mock 네이티브==컬럼) (got {len(hits)})")
        finally:
            import psycopg
