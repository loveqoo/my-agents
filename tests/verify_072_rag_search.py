...
_fails: list[str] = []
CP = "col-v072-"  # 스펙 148 이름 규칙(dash-only) 준수 — 구 `_` 접두는 400
MP = "mdl-v072-"
...
async def main() -> None:
    ...
    try:
        async with httpx.AsyncClient(
            ...
            empty_id = next(
                x["id"] for x in (await c.get("/collections")).json() if x["name"] == f"{CP}empty"
            )
            ...
            check(
                scores and abs(scores[0] - 1.0) < 1e-3,
                f"A1 exact-match rank1 score≈1.000 (got {scores[:1]})",
            )
            check(
                all({"score", "filename", "text"} <= set(h) for h in hits),
                "A1 hit 구조(score/filename/text)",
            )
            check(scores == sorted(scores, reverse=True), f"A2 score 내림차순 (got {scores})")
            check(
                len(scores) >= 2 and all(s < 1.0 - 1e-9 for s in scores[1:]),
                f"A3 비-exact score<1 (got {scores})",
            )
            ...
            try:
                ...
                check(
                    exc.kind == "empty",
                    f"A5 빈 질의 → RagSearchError(kind=empty) (got kind={exc.kind})",
                )
            ...
            check(
                bool(ms) and abs(ms[0] - 1.0) < 1e-3, f"A7 멀티 컬렉션 exact rank1 (got {ms[:1]})"
            )
            ...
            j1 = r1.json()
            check(
                isinstance(j1.get("results"), list) and bool(j1["results"]),
                "B1 results 리스트 비어있지 않음",
            )
            top = j1["results"][0]
            check({"score", "filename", "text"} <= set(top), f"B1 결과 구조 (got {list(top)})")
            check(
                abs(top["score"] - 1.0) < 1e-3, f"B1 exact rank1 score≈1.000 (got {top['score']})"
            )
            check(
                j1["results"] == sorted(j1["results"], key=lambda h: h["score"], reverse=True),
                "B1 응답 score 내림차순",
            )
            ...
            check(
                secret and secret not in r1.text and "api_key" not in r1.text,
            ...
            r6 = await c.post(
                f"/collections/{main_id}/search", json={"query": "가" * 4001, "top_k": 4}
            )
            check(r6.status_code == 422, f"B6 거대 query(>4000) → 422 (got {r6.status_code})")
            r6b = await c.post(
                f"/collections/{main_id}/search", json={"query": "가" * 4000, "top_k": 4}
            )
            ...
            check(
                r7.status_code == 400,
                f"B7 chat 모델 컬렉션 검색 → 400(502 아님) (got {r7.status_code})",
            )
    finally:
        await _cleanup()
