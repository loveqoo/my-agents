...
check(selected_for_server("web-fetch", ["calc-tools__add"]) is None, "타 서버 항목 → 이 서버 전체 폴백")

# ④ 혼합 선택은 서버별로 분할
sel = ["calc-tools__add", "web-fetch__wiki_search"]
check(selected_for_server("calc-tools", sel) == {"calc-tools__add"}, "혼합 — calc-tools는 add만")
check(selected_for_server("web-fetch", sel) == {"web-fetch__wiki_search"}, "혼합 — web-fetch는 wiki_search만")

# ⑤ 대시 서버명 접두 혼동 없음(NAME_RULE 밑줄 금지 — 272 불변식 공유)
check(selected_for_server("calc", ["calc-tools__add"]) is None, "'calc'는 'calc-tools__' 접두에 안 걸림")
...
check(dumped["tools"] == ["calc-tools__add"] and dumped["mcps"] == ["calc-tools"], "AgentConfig 왕복 보존")
