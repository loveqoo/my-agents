...
def _provision(create: bool) -> None:
    args = (
        [PY, PROV, "create", MEMBER_EMAIL, PW, "member"]
        if create
...
async def _chat_session_echo(
    client: httpx.AsyncClient, agent_uuid: str, session_id: str
) -> str | None:
...
async def _row(s, sid: str):
    return (
        await s.execute(select(Session.user_id, Session.id).where(Session.session_id == sid))
    ).first()


async def _msgcount(s, session_pk) -> int:
    return (
        await s.execute(select(func.count(Message.id)).where(Message.session_pk == session_pk))
    ).scalar_one()
...
async def main() -> None:
    ...
    try:
        ...
        async with SessionLocal() as s:
            agent_uuid = str(
                (await s.execute(select(Agent.id).where(Agent.agent_id == AGENT_ID))).scalar_one()
            )
        ...
        try:
            ...
            check(
                echo_v is not None and not str(echo_v).startswith("HTTP_"),
            ...
            check(str(echo_v).startswith("sess-"), f"T1: 발급된 세션은 새 sess-* id — got {echo_v}")
            ...
            check(
                n_uid1 is None and n_msgs1 == n_msgs0,
            ...
            check(b_uid1 == member_id, f"T6: B 본인 세션 소유자 유지 — got {b_uid1}")
            ...
            async with SessionLocal() as s:
                agent_pk = (
                    await s.execute(select(Agent.id).where(Agent.agent_id == AGENT_ID))
                ).scalar_one()
            ...
            await _create_approval(
                apr_ctx,
                "thr-068t",
                {"permission": "data.read", "action": "test", "args": {}, "summary": "s"},
                member_id,
            )
            async with SessionLocal() as s:
    ...
    if _fails:
        ...
        sys.exit(1)
    print(
        "✅ 스펙 068 라이브 통합 — chat resume 오라클 제거·탈취 봉인·NULL 차단·본인 resume 무회귀 통과"
    )
