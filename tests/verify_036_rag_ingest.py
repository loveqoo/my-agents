...
async def _cleanup() -> None:
    async with SessionLocal() as s:
        ...
        await s.execute(
            delete(ModelConfig).where(ModelConfig.name.like(f"{MP}%"))
        )  # 컬렉션 제거 후라 RESTRICT 해제
        await s.commit()
...
async def main() -> None:
    ...
    try:
        async with httpx.AsyncClient(
            ...
            mock_chat = next(
                (m for m in chat_m if m["kind"] == "chat" and "mock" in m["name"].lower()), None
            )
            mk = await c.post(
                "/models",
                json={
            ...
            r = await c.post(
                "/collections", json={"name": f"{CP}bad1", "embedding_model_id": str(uuid.uuid4())}
            )
            check(r.status_code == 400, f"없는 임베딩 모델 → 400 (got {r.status_code})")
            if mock_chat:
                r = await c.post(
                    "/collections",
                    json={"name": f"{CP}bad2", "embedding_model_id": mock_chat["id"]},
                )
            ...
            check(
                doc["status"] == "ready",
                f"문서 status=ready (got {doc['status']}, err={doc.get('error')})",
            )
            ...
            async with SessionLocal() as s:
                ch = (
                    await s.execute(
                        select(Chunk).where(Chunk.collection_id == uuid.UUID(cid)).limit(1)
                    )
                ).scalar_one()
            ...
            check(
                h1["db_dims"] == 1024
                and h1["collection_dims"] == 1024
                and h1["model_dims"] == 1024,
            ...
            check(
                h2["collection_dims"] == 999 and h2["db_dims"] == 1024,
                "health가 drift 노출(999 vs 1024)",
            )
