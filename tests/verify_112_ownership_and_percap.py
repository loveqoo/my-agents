...
def unit_checks() -> None:
    ...
    check(
        next_owner("a", "b") == "a" and next_owner(None, "b") == "b",
        "U1 next_owner=기존 보존(이전 금지)",
    )
    # 스펙 182 통합 경계: 빈 incoming은 current 보존, 동일 유저는 부여, current==""도 기존으로 보존(fail-closed).
    check(
        next_owner("a", "") == "a"
        and next_owner("a", None) == "a"
        and next_owner("a", "a") == "a"
        and next_owner("", "b") == ""
        and next_owner(None, None) is None,
    ...
    check(
        is_first_party("ui")
        and is_first_party("code")
        and not is_first_party("external")
        and is_third_party("external")
        and not is_third_party("ui")
        and not is_third_party("code")
        and all(is_first_party(x) != is_third_party(x) for x in ("ui", "code", "external")),
        "U1 source 축: ui/code=제1자·external=제3자·여집합",
    )
    check(
        may_use("bob", "bob", False)
        and not may_use("x", "bob", False)
        and not may_use(None, "bob", False)
        and may_use(None, "bob", True),
        "U1 may_use: 소유자 OK·타인 X·NULL은 특권만",
    )
    check(
        is_privileged("mat")
        and is_privileged(Stub(is_superuser=True))
        and not is_privileged(Stub()),
    ...
    b_kind = PolicyScopedBroker(
        {"rag:a", "rag:b"},
        lambda k, name=None: True,
        providers=build_providers(BrokerContext(session_factory=lambda: None)),
    )
    check(
        b_kind._permitted("rag:a") and b_kind._permitted("rag:b"),
        "U3 kind 허가 → 모든 rag cap permitted",
    )
    ...
    b_pc = PolicyScopedBroker(
        {"rag:a", "rag:b"},
        only_a,
        providers=build_providers(BrokerContext(session_factory=lambda: None)),
    )
    check(
        b_pc._permitted("rag:a") and not b_pc._permitted("rag:b"),
...
async def _rbac_check_real() -> None:
    ...
    try:
        ...
        check(
            _rbac_allows(e, uid, "mcp", "github/search"),
            "U5 mcp 서버단위 부여 → 툴 호출 True(서버 폴백)",
        )
        check(_rbac_allows(e, uid, "mcp", "github"), "U5 mcp 서버 자체 → True")
        check(
            not _rbac_allows(e, uid, "mcp", "gitlab/search"), "U5 다른 서버 툴 → False(폴백 정확)"
        )
    finally:
    ...
    try:
        async with httpx.AsyncClient(transport=t, base_url="http://t", timeout=60) as c:
            ...
            async with SessionLocal() as db:
                row = (
                    await db.execute(select(Agent).where(Agent.id == uuid.UUID(aid)))
                ).scalar_one()
                check(
                    row.owner_id == str(alice.id), f"H1 owner_id 스탬프=alice (got {row.owner_id})"
                )
            ...
            r_upd = await c.put(
                f"/agents/{aid}",
                json={"config": {"model": "mock-llm", "prompt": "x", "historyDepth": 6}},
            )
            check(
                r_upd.status_code == 404, f"H2 타 member 수정 → 404-fold (got {r_upd.status_code})"
            )
            r_del = await c.delete(f"/agents/{aid}")
            check(
                r_del.status_code == 404, f"H2 타 member 삭제 → 404-fold (got {r_del.status_code})"
            )
            ...
            r_own = await c.put(
                f"/agents/{aid}",
                json={"config": {"model": "mock-llm", "prompt": "mine", "historyDepth": 6}},
            )
            check(
                r_own.status_code == 200, f"H3 자가잠금 핀: 본인 수정 OK (got {r_own.status_code})"
            )
            ...
            r_adm = await c.put(
                f"/agents/{aid}",
                json={"config": {"model": "mock-llm", "prompt": "adm", "historyDepth": 6}},
            )
            check(
                r_adm.status_code == 200,
                f"H4 특권(superuser) 타인 것 수정 OK (got {r_adm.status_code})",
            )

            # NULL-owned(레거시): owner_id=None 에이전트 직접 삽입 → member 거부, 특권 OK.
            async with SessionLocal() as db:
                legacy = Agent(
                    agent_id=f"agt_legacy_{uuid.uuid4().hex[:6]}",
                    name="legacy",
                    source="ui",
                    model="mock-llm",
                    prompt="",
            ...
            check(
                r_leg.status_code == 404,
                f"H5 NULL-owned member 수정 → 404(admin 전용 fail-closed) (got {r_leg.status_code})",
            )
            _as(admin)
            r_leg2 = await c.put(
                f"/agents/{legacy_id}",
                json={"config": {"model": "mock-llm", "prompt": "ok", "historyDepth": 6}},
            )
            check(
                r_leg2.status_code == 200, f"H5 NULL-owned 특권 수정 OK (got {r_leg2.status_code})"
            )
            ...
            check(
                r_madd.status_code == 404
                and r_mpatch.status_code == 404
                and r_mdel.status_code == 404,
            ...
            r_missing = await c.put(
                f"/agents/{_uuid.uuid4()}",
                json={"config": {"model": "mock-llm", "prompt": "x", "historyDepth": 6}},
            )
            r_notmine = await c.put(
                f"/agents/{aid}",
                json={"config": {"model": "mock-llm", "prompt": "x", "historyDepth": 6}},
            )
            check(
                r_missing.status_code == 404
                and r_notmine.status_code == 404
