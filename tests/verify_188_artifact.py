...
def unit_checks() -> None:
    ...
    s = summarize_artifact(
        Artifact(kind="travel-request", data={"목적지": "서울", "예산": "50만원"})
    )
...
def graph_checks() -> None:
    ...
    def _run(agent, tid: str, answers: list[str]) -> tuple[dict, list[str]]:
        ...
        result = asyncio.run(
            graph.ainvoke(
                {"messages": [{"role": "user", "content": "출장 신청할게"}]}, config=_cfg(tid)
            )
        )
        i = 0
    ...
    check(
        last is not None and "산출물 완성" in str(getattr(last, "content", "")),
        "A4 최종 요약 메시지",
    )
    ...
    r = asyncio.run(
        graph.ainvoke({"messages": [{"role": "user", "content": "go"}]}, config=_cfg("t-replay"))
    )
    r = asyncio.run(
        graph.ainvoke(Command(resume={"type": "text", "message": "답1"}), config=_cfg("t-replay"))
    )
    r = asyncio.run(
        graph.ainvoke(Command(resume={"type": "text", "message": "답2"}), config=_cfg("t-replay"))
    )
    ...
    check(
        questions.count(questions[0].split(" (")[0]) >= 1 and len(questions) >= 5, "B1 재질문 발생"
    )
    art = result.get("artifact")
    ...
    r = asyncio.run(
        graph.ainvoke({"messages": [{"role": "user", "content": "시작"}]}, config=_cfg("t-form"))
    )
    ...
    r = asyncio.run(
        graph.ainvoke(
            Command(resume={"type": "form", "values": {"gender": "남성", "city": "화성"}}),
            config=_cfg("t-form"),
        )
    )
    p1 = r["__interrupt__"][0].value
    check(
        p1.get("kind") == "form" and p1["prefill"] == {"gender": "남성"} and "note" in p1,
        "FM2 후보밖 값 탈락→갱신 프리필로 재제시+note",
    )
    # 이중 입력: 폼 대기 중 텍스트 → 후보 매칭 병합 → 전부 참 → 완료
    r = asyncio.run(
        graph.ainvoke(
            Command(resume={"type": "text", "message": "부산이야"}), config=_cfg("t-form")
        )
    )
    check(
        r.get("artifact", {}).get("data") == {"gender": "남성", "city": "부산"},
    ...
    r = asyncio.run(
        g2.ainvoke({"messages": [{"role": "user", "content": "부산 살아"}]}, config=_cfg("t-cf"))
    )
    check(r["__interrupt__"][0].value.get("kind") == "form", "FC1 confirm 폼 제시")
    # 텍스트로 부산을 채워 필수가 전부 차더라도 confirm이면 **재제시**(무확인 확정 봉인).
    r = asyncio.run(
        g2.ainvoke(
            Command(resume={"type": "text", "message": "부산으로 해줘"}), config=_cfg("t-cf")
        )
    )
    check(
        "__interrupt__" in r and "artifact" not in r, "FC2 텍스트 병합 후 재제시(무확인 확정 안 함)"
    )
    check(
        r["__interrupt__"][0].value.get("prefill") == {"city": "부산"}, "FC3 재제시 프리필=병합값"
    )
    # 폼 제출(명시 확정)로만 최종 확정.
    r = asyncio.run(
        g2.ainvoke(
            Command(resume={"type": "form", "values": {"city": "부산"}}), config=_cfg("t-cf")
        )
    )
    ...
    try:
        asyncio.run(
            bad_graph.ainvoke(
                {"messages": [{"role": "user", "content": "x"}]}, config=_cfg("t-bad")
            )
        )
