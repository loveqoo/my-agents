...
def unit_checks() -> None:
    ...
    check(len(sink) == 1 and sink[0]["status"] == "error", f"U1 실패가 status=error 기록 (got {sink[:1]})")
    err = sink[0].get("error", "")
    check("RuntimeError" in err and "연결 거부" in err,
    ...
    check(isinstance(inv.get("error"), str) and "capability not found" in inv["error"],
    ...
    check("비어" in sink3[0].get("error", ""), f"U3 RAG error에 사람 사유(tool_msg) (got {sink3[0].get('error')!r})")
    ...
    check("leaked_abcdef123456" not in e4 and "«secret»" in e4,
          f"U4 사유 속 토큰 마스킹 (got {e4!r})")
    ...
    check(len(sink5[0].get("error", "")) <= runtime._ERR_CAP + 1,
    ...
    rt6 = StructuredTool.from_function(
        coroutine=_isError, name="isErr", description="t",
    ...
    check(sink6[0]["status"] == "error" and "의도된 실패" in sink6[0].get("error", ""),
    ...
    rt6b = StructuredTool.from_function(coroutine=_ok6, name="ok6", description="t",
                                        handle_tool_error=lambda e: "swallowed")
    sink6b: list[dict] = []
    asyncio.run(runtime._wrap_mcp_tool("srv", rt6b, sink6b, None).ainvoke({"x": "y"}))
    check(sink6b[0]["status"] == "ok" and "error" not in sink6b[0], "U6b 성공 경로 무회귀(status=ok)")
    ...
    rt7 = StructuredTool.from_function(coroutine=_bfail, name="bfail", description="t",
    ...
    check(res.error and "의도된 실패" in res.error,
          f"U7 브로커 MCP 실패 사유 보존(res.error) (got {res.error!r})")
    # U7b 브로커 성공 경로 무회귀 — 정상 반환은 error=None.
    async def _bok(reason: str = "") -> str:
        return "정상"

    rt7b = StructuredTool.from_function(coroutine=_bok, name="bok", description="t",
                                        handle_tool_error=lambda e: "swallowed")
    res_b = asyncio.run(McpProvider(None).invoke(_McpBacking("srv", "bok", rt7b, None), {"reason": "x"}))
    ...
    check("s3cr3tPass" not in leak_url and "alice:s3cr3tPass" not in leak_url and "example.com" in leak_url,
