...
check(f({}) is False, "hits_lte rag 관측 부재(agent 런) → False(fail-closed — 빈 관측 0<=N 위장 차단)")
...
check(gg({"rag": {"hits": [1, 2]}}) is True and gg({"rag": {"hits": [1]}}) is False, "gte 무회귀(2>=2 통과·1>=2 실패)")
