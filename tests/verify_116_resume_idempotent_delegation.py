...
async def main_checks():
    ...
    ctx = AgentBuildContext(
        prompt="오케스트레이터",
        model_cfg={"base_url": "http://x", "model_id": "mock-llm"},
        tools=[],
        broker=broker,
    ...
    check(
        interrupted is not None and interrupted.get("cap") == GATED,
        "H1 GATED에서 interrupt(전송 이전)",
    )
    check(
        broker.counts.get(READ) == 1,
        f"H1 pause 시점 READ invoke 1회 (got {broker.counts.get(READ)})",
    )
    ...
    check(
        broker.sideeffects == 1,
        f"H2 GATED 부수효과 정확히 1회(재실행돼도, 스펙 101 보존) (got {broker.sideeffects})",
    )
    ...
    check(
        "읽기결과" in delegated and "게이트 실행됨" in delegated,
    ...
    g4 = FixedOrder().build_graph(
        AgentBuildContext(
            prompt="o",
            model_cfg={"base_url": "http://x", "model_id": "m"},
            tools=[],
            broker=b4,
            checkpointer=MemorySaver(),
        )
    ...
    async for mode, chunk in g4.astream(
        {"messages": [("user", "요청")]}, config=c4, stream_mode=["updates"]
    ):
        pass
    check(b4.discover_calls == 1, f"H4 초기 실행 discover 1회 (got {b4.discover_calls})")
    try:
        async for _ in g4.astream(
            Command(resume={"decision": "approve"}), config=c4, stream_mode=["updates"]
        ):
            pass
    ...
    first = None
    async for mode, chunk in g5.astream(
        {"messages": [("user", "요청")]}, config=c5, stream_mode=["updates"]
    ):
        if mode == "updates" and isinstance(chunk, dict) and "__interrupt__" in chunk:
    ...
    second = None
    async for mode, chunk in g5.astream(
        Command(resume={"decision": "approve"}), config=c5, stream_mode=["updates"]
    ):
        if mode == "updates" and isinstance(chunk, dict) and "__interrupt__" in chunk:
            second = chunk["__interrupt__"][0].value
    check(
        second and second.get("cap") == GATED2,
