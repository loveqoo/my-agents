...
check(len(t3["mcps"]) == 20 and all(len(x) <= 101 for x in t3["mcps"]), f"V3 리스트 20개·항목 100캡 (got {len(t3['mcps'])})")

check(_overrides_trace({}) is None and _overrides_trace(None) is None and _overrides_trace("x") is None,
...
check(_overrides_trace({"systemPrompt": "   "}) is None, "V5 공백 systemPrompt → 미기록(적용 가드 미러)")
check(_overrides_trace({"systemPrompt": "   ", "model": "m"}) == {"model": "m"}, "V5b 다른 키는 유지")
