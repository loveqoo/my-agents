...
async def main() -> None:
    ...
    try:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://t", headers=_AUTH) as c:
            ...
            d1 = r1.json()
            check(
                r1.status_code == 200 and set(d1) >= {"items", "total"} and d1["total"] == 26,
            ...
            check(
                not ids1 & {it["id"] for it in d2["items"]},
            ...
            check(
                r4.json()["total"] == 1 and "a%b_c" in r4.json()["items"][0]["filename"],
                f"D4a 와일드카드 리터럴 매치 (got {r4.json()['total']})",
            )
            r5 = await c.get(base, params={"q": "%", "limit": 20, "offset": 0})
            check(
                r5.json()["total"] == 1, f"D4b '%' 단독 → 특수문자 행만 (got {r5.json()['total']})"
            )
            ...
            r8 = await c.get(
                f"/collections/{_uuid.uuid4()}/documents", params={"limit": 20, "offset": 0}
            )
