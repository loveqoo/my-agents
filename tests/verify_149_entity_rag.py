...
async def main():
    ...
    check(
        "title: 상품A" in t and 'tags: ["x", "y"]' in t, f"V1b data=객체 평탄화(중첩 JSON): {t!r}"
    )
    good = (
        b"\n".join(
            [
                json.dumps(
                    {
                        "metadata": {"pid": i, "sid": i * 10},
                        "data": {"name": f"상품{i}", "desc": f"설명 {i}"},
                    },
                    ensure_ascii=False,
                ).encode()
                for i in range(3)
    ...
    async with async_session() as s:
        try:
            ...
            check(
                e.status_code == 400 and "Schema" in str(e.detail),
                f"V2a 무효 스키마 400 (got {e.status_code})",
            )
    async with async_session() as s:
        try:
            await RG.create_collection(
                CollectionIn(
                    name=f"{tag}-doc-schema",
                    kind="document",
    ...
    async with async_session() as s:
        try:
            ...
            check(
                e.status_code == 400 and "pattern" in str(e.detail),
                f"V2e 정규식 키워드 400 (got {e.status_code})",
            )
    async with async_session() as s:
        ...
        col_id = col.id
        check(
            col.kind == "entity" and col.entity_schema == schema,
            "V2c 엔티티 컬렉션 생성(kind+스키마 저장)",
        )
    async with async_session() as s:
        # 명시적 null=스키마 제거 → 재등록(codex 149 — 오등록 스키마 해제 경로)
        out = await RG.update_collection(
            col_id, CollectionUpdate(entity_schema=None), session=s, principal=admin
        )
        check(out.entity_schema is None, "V2f 명시적 null로 스키마 제거")
    async with async_session() as s:
        out = await RG.update_collection(
            col_id, CollectionUpdate(entity_schema=schema), session=s, principal=admin
        )
        check(out.entity_schema == schema, "V2g 스키마 재등록")
    async with async_session() as s:
        doc_cols = (
            (await s.execute(select(Collection).where(Collection.kind == "document")))
            .scalars()
            .all()
        )
        target_doc = next((c for c in doc_cols if c.name == "docs-kb"), None)
        try:
            await RG.update_collection(
                _uuid.UUID(str(target_doc.id)) if target_doc else col_id,
                CollectionUpdate(entity_schema=schema),
                session=s,
                principal=admin,
            )
    ...
    try:
        ...
        async with async_session() as s:
            try:
                await RG.ingest_document(
                    col_id, file=_upload("bad.jsonl", bad_line), session=s, principal=admin
                )
                check(False, "V3a 스키마 위반 업로드 400이어야")
            except HTTPException as e:
                check(
                    e.status_code == 400 and "1번째 줄" in str(e.detail),
                    f"V3a 위반 업로드 400+행 번호 (got {e.status_code}: {str(e.detail)[:40]})",
                )
        async with async_session() as s:
            n_docs = (
                (await s.execute(select(Document).where(Document.collection_id == col_id)))
                .scalars()
                .all()
            )
        ...
        async with async_session() as s:
            doc = await RG.ingest_document(
                col_id, file=_upload("entities.jsonl", ent_file), session=s, principal=admin
            )
            _doc_id = doc.id
        ...
        async with async_session() as s:
            chunks = (
                (
                    await s.execute(
                        select(Chunk).where(Chunk.collection_id == col_id).order_by(Chunk.ordinal)
                    )
                )
                .scalars()
                .all()
            ...
            check(
                len(chunks) == 3 and chunks[0].meta == {"pid": 1, "order_id": "o-1"},
                "V3d 1행=1청크 + meta 저장",
            )
            check(
                "name: 무선 키보드 1" in chunks[0].text,
                f"V3e 임베딩 텍스트=평탄화: {chunks[0].text[:40]!r}",
            )
        ...
        check(
            len(hits) >= 1 and isinstance(hits[0].get("meta"), dict) and "pid" in hits[0]["meta"],
            f"V4a 엔티티 hit meta 동반: {hits[0].get('meta')}",
        )
        async with async_session() as s:
            docs_kb = (
                await s.execute(select(Collection).where(Collection.name == "docs-kb"))
            ).scalar_one_or_none()
        ...
        txt2 = RT.format_rag_hits(
            [{"score": 0.9, "filename": "a.md", "text": "본문", "meta": None}]
        )
