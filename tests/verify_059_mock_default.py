...
def main() -> None:
    ...
    check(
        'name="Mock LLM"' in ssrc and 'kind="mock"' in ssrc, "D2 Mock LLM provider(kind=mock) 시드"
    )
    check(
        'model_id="mock-chat"' in ssrc and 'kind="chat"' in ssrc and "is_default=True" in ssrc,
        "D2 mock-chat 채팅 모델 is_default 시드",
    )
    check(
        'name="mock-embed"' in ssrc
        and 'model_id="mock-embed"' in ssrc
        and 'kind="embedding"' in ssrc,
        "D2 mock-embed 임베딩 모델 시드",
    )
    # 에이전트 모델 참조가 CHAT_MODEL_NAME 단일 소스 → 시드 chat 모델 이름과 일치(댕글링 0).
    check(
        "CHAT_MODEL_NAME" in ssrc and ssrc.count("CHAT_MODEL_NAME") >= 3,
    ...
    check(
        'down_revision: Union[str, Sequence[str], None] = "b7c8d9e0f1a2"' in mig,
        "D1 down_revision이 직전 head(b7c8d9e0f1a2)",
    )
    check(
        "if row is None:" in mig and "return" in mig, "D1 mock-llm 부재 시 무동작(예외 상태 비침습)"
    )
    # no-clobber: chat 기본이 이미 있으면 승격 안 함.
    check(
        "has_default_chat" in mig and "if not has_default_chat:" in mig,
    ...
    check(
        "embed_exists" in mig and "if not embed_exists:" in mig,
        "D1 mock-embed 멱등(이름 존재 시 INSERT skip)",
    )
    check("has_default_embed" in mig, "D1 임베딩 기본도 '없을 때만' default(임베딩 no-clobber)")
    # 가역 downgrade.
    dwn = mig[mig.index("def downgrade") :]
    check("DELETE FROM models WHERE name = 'mock-embed'" in dwn, "D1 downgrade가 mock-embed 제거")
    check(
        "SET is_default = false WHERE name = 'mock-llm'" in dwn,
    ...
    check(
        "MLX_BASE_URL" not in env and "MLX_API_KEY" not in env and "MLX_MODEL" not in env,
        "D3 .env.example에 MLX_* env 0건",
    )
    check(
        "Mock LLM" in env and "Provider UI" in env, "D3 .env.example에 Mock 기본 + Provider UI 안내"
    )

    # ── D4. agent CLI + 스키마/모델 기본값 ──────────────────────────────────────
    asrc = inspect.getsource(agent_main.main)
    check(
        "MODEL_BASE_URL" in asrc and "MODEL_ID" in asrc, "D4 agent CLI가 벤더무관 MODEL_* env 사용"
    )
    full_agent = inspect.getsource(agent_main)
    check(
        "MLX_BASE_URL" not in full_agent
        and "MLX_API_KEY" not in full_agent
    ...
    check(
        "기본은 MLX" not in hsrc and "MLX_BASE_URL" not in hsrc, "D5 힌트에서 'MLX 기본' 안내 제거"
    )
    ...
    check(
        h is not None and "Mock LLM" in h and "http://localhost:8045/v1" in h,
        "D5 연결실패+cfg → Mock LLM 힌트(base_url 포함, 무회귀)",
    )
    check(
        chat._model_error_hint(Exception("model not found (404)"), cfg) is None,
        "D5 비연결오류(404)는 힌트 없음(무회귀)",
    )
    check(
        chat._model_error_hint(Exception("x"), None) is None,
    ...
    rm = _read("README.md")
    check(
        "`MLX_BASE_URL` / `MLX_API_KEY` / `MLX_MODEL`" not in rm,
        "D6 README 환경변수 표에서 MLX_* 행 제거",
    )
    check("Mock LLM" in rm and "Provider UI" in rm, "D6 README에 Mock 기본 + Provider UI 안내")
