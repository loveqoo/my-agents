...
async def main() -> None:
    ...
    def mk_agent(owner):
        a = Agent(
            agent_id=f"e178_{uuid.uuid4().hex[:6]}",
            name=f"e178-{uuid.uuid4().hex[:4]}",
            source="ui",
            model="mock-chat",
            prompt="",
            history_depth=5,
    ...
    try:
        ...
        async with SessionLocal() as s:
            ds = await ER.create_dataset(
                DatasetIn(name=f"e178-ds-{uuid.uuid4().hex[:5]}", kind="agent"), session=s, user=A
            )
            ds_id = ds.id
            made_ds.append(ds_id)
        async with SessionLocal() as s:
            case = await ER.create_case(
                ds_id, CaseIn(name="c1", input="hi", asserts=[], order_idx=0), session=s, user=A
            )
            case_id = case.id
        ...
        async with SessionLocal() as s:
            await _expect_status(
                ER.update_dataset(ds_id, DatasetIn(name="x", kind="agent"), session=s, user=B),
                404,
                "T2 B가 A dataset 수정",
            )
        async with SessionLocal() as s:
            await _expect_status(
                ER.delete_dataset(ds_id, session=s, user=B), 404, "T2 B가 A dataset 삭제"
            )
        async with SessionLocal() as s:
            await _expect_status(
                ER.create_case(
                    ds_id, CaseIn(name="x", input="x", asserts=[], order_idx=1), session=s, user=B
                ),
                404,
                "T2 B가 A dataset에 케이스 추가",
            )
        async with SessionLocal() as s:
            await _expect_status(
                ER.update_case(
                    case_id, CaseIn(name="x", input="x", asserts=[], order_idx=0), session=s, user=B
                ),
                404,
                "T2 B가 A case 수정",
            )
        async with SessionLocal() as s:
            await _expect_status(
                ER.delete_case(case_id, session=s, user=B), 404, "T2 B가 A case 삭제"
            )
        async with SessionLocal() as s:
            await _expect_status(
                ER.start_run(ds_id, RunStartIn(agent_id=aA_id), session=s, user=B),
                404,
                "T2 B가 A dataset 실행",
            )
        async with SessionLocal() as s:
            await _expect_status(
                ER.suggest_cases(ds_id, ER.SuggestIn(agent_id=aA_id), session=s, user=B),
                404,
                "T2 B가 A dataset 출제",
            )
        ...
        async with SessionLocal() as s:
            out = await ER.update_dataset(
                ds_id,
                DatasetIn(name=f"e178-ds2-{uuid.uuid4().hex[:5]}", kind="agent"),
                session=s,
                user=A,
            )
            check(out.id == ds_id, "T3 소유자 A는 본인 dataset 수정 통과")
        async with SessionLocal() as s:
            c2 = await ER.create_case(
                ds_id, CaseIn(name="c2", input="hi2", asserts=[], order_idx=1), session=s, user=A
            )
        ...
        async with SessionLocal() as s:
            await _expect_status(
                ER.start_run(ds_id, RunStartIn(agent_id=aB_id), session=s, user=A),
                404,
                "T4 A가 남의 private 에이전트로 실행",
            )
        async with SessionLocal() as s:
            await _expect_status(
                ER.suggest_cases(ds_id, ER.SuggestIn(agent_id=aB_id), session=s, user=A),
                404,
                "T4 A가 남의 private 에이전트로 출제",
            )
        ...
        async with SessionLocal() as s:
            out = await ER.update_dataset(
                ds_id,
                DatasetIn(name=f"e178-admin-{uuid.uuid4().hex[:5]}", kind="agent"),
                session=s,
                user=ADMIN,
            )
        ...
        async with SessionLocal() as s:
            base = (
                await s.execute(
                    ...
                    .select(__import__("sqlalchemy").func.count(EvalCase.id))
            ...
            for i in range(61 - base):
                s.add(
                    EvalCase(
                        dataset_id=ds_id, name=f"bulk{i}", input="x", asserts=[], order_idx=100 + i
                    )
                )
            await s.commit()
        async with SessionLocal() as s:
            await _expect_status(
                ER.start_run(ds_id, RunStartIn(agent_id=aA_id), session=s, user=A),
                422,
                "T6 작업량(cases×models>60) 상한 → 422",
            )
        async with SessionLocal() as s:
            # 특권은 상한 우회
            try:
        ...
        async with SessionLocal() as s:
            ds2 = EvalDataset(
                name=f"e178-oth-{uuid.uuid4().hex[:5]}", kind="agent", owner_id=str(A.id)
            )
            s.add(ds2)
            await s.commit()
            made_ds.append(ds2.id)
            for _ in range(2):
                r = EvalRun(
                    dataset_id=ds2.id,
                    agent_pk=aA_id,
        ...
        async with SessionLocal() as s:
            small = await ER.create_dataset(
                DatasetIn(name=f"e178-sm-{uuid.uuid4().hex[:5]}", kind="agent"), session=s, user=A
            )
            made_ds.append(small.id)
            small_id = small.id
        async with SessionLocal() as s:
            await ER.create_case(
                small_id, CaseIn(name="c", input="hi", asserts=[], order_idx=0), session=s, user=A
            )
        async with SessionLocal() as s:
            await _expect_status(
                ER.start_run(small_id, RunStartIn(agent_id=aA_id), session=s, user=A),
                429,
                "T6 동시 run 상한(2) 초과 → 429",
            )
        ...
        try:
            async with SessionLocal() as s:
                await _expect_status(
                    ER._member_job_guard(s, A), 429, "T7 A 배경작업 2개 in-flight → 429"
                )
            ...
            ER._active_jobs.discard(j1.id)
            ER._active_jobs.discard(j2.id)
        ...
        async with SessionLocal() as s:
            jds = await ER.create_dataset(
                DatasetIn(name=f"e178-jw-{uuid.uuid4().hex[:5]}", kind="agent"), session=s, user=A
            )
            made_ds.append(jds.id)
            jds_id = jds.id
        async with SessionLocal() as s:
            for i in range(11):
                s.add(
        ...
        async with SessionLocal() as s:
            await _expect_status(
                ER.start_run(jds_id, RunStartIn(agent_id=aA_id), session=s, user=A),
                422,
                "T9 judge 포함 work>60 → 422",
            )
        ...
        async with SessionLocal() as s:
            ...
            for did in made_ds:
                ...
                if d:
                    await s.delete(d)  # cases/runs CASCADE
            for aid in made_ag:
