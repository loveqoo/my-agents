...
def main() -> None:
    ...
    srv = cli.post(
        "/mcp-servers",
        json={
            "name": f"v371-{tag}", "source": "local", "transport": "http", "url": lt.get("url"),
            "tools": ["failing_op", "echo"], "enabled_tools": ["failing_op", "echo"],
        },
    ).json()
    p = cli.post("/prompts", json={"name": f"v371p-{tag}", "tone": "t", "body": "너는 V371 검증봇이다."}).json()
    a = cli.post(
        "/agents",
        json={
            "name": f"v371a-{tag}",
            "config": {
                "model": "mock-llm", "prompt": f"v371p-{tag}", "memories": [],
                "vectorTables": [], "mcps": [f"v371-{tag}"], "historyDepth": 10,
            },
        },
    ).json()
    aid = a["id"]
    ...
    try:
        ...
        check(b2.get("graph", 99) <= 0.5, f"V1 2턴째 graph ≤0.5ms(재컴파일 0) (got {b2.get('graph')})")
        check(b2.get("mcp", 99) <= 0.5, f"V1 2턴째 mcp ≤0.5ms(재디스커버리 0) (got {b2.get('mcp')})")
        ...
        check("V371 검증봇" in (sysmsgs[0].get("content") or "") if sysmsgs else False, "V3 system에 프롬프트 본문")
        ...
        async def race() -> list[dict | None]:
            async with httpx.AsyncClient(base_url=BASE, timeout=120.0, cookies=cli.cookies) as ac:
                async def one(i: int) -> dict | None:
                    r = await ac.post(
                        f"/agents/{aid}/chat",
                        json={"messages": [{"role": "user", "content": f"실패 테스트 ISOLATE-{i}"}]},
                    )
                    return sse_trace(r.text)
        ...
        for i, tr in enumerate(traces):
            ...
            mine = [c for c in fired if f"ISOLATE-{i}" in json.dumps(c.get("args") or {}, ensure_ascii=False)]
            if len(fired) == 1 and len(mine) == 1:
                iso_ok += 1
        check(iso_ok == 8, f"V2 동시 8턴 트레이스 격리(자기 호출 1건만) — {iso_ok}/8")

        # V4 — 블록 편집(새 버전) → 채택 → 오픈 → 새 도구 목록 반영(새 키)
        srv_edit = {"name": f"v371-{tag}", "source": "local", "transport": "http", "url": lt.get("url"),
        ...
        fired4 = [c for c in (t4 or {}).get("mcp") or [] if "failing_op" in str(c.get("tool") or "")]
        check(len(fired4) == 0, f"V4 채택+오픈 후 failing_op 미발동(새 버전=enabled서 제거) (got {len(fired4)})")
    finally:
