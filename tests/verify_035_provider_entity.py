    ...
    try:
        async with httpx.AsyncClient(transport=transport, base_url="http://t", headers=_AUTH) as c:
            ...
            if provs:
                p0 = provs[0]
                check(
                    set(p0) >= {"id", "name", "protocol", "base_url", "api_key", "modelCount"},
                    f"provider 키 집합 (got {sorted(p0)})",
                )
            if mods:
                m0 = mods[0]
                check(
                    {"provider_id", "provider_name", "base_url"} <= set(m0),
            ...
            pid = pc.json()["id"]
            check(
                _secret_safe(pc.json()["api_key"]) and pc.json()["api_key"] == SECRET_MASK,
                "생성 직후 키 마스킹",
            )
            ...
            mid = mc.json()["id"]
            check(
                mc.json()["provider_id"] == pid
                and mc.json()["base_url"] == "http://127.0.0.1:65535/v1",
            ...
            check(
                d1.status_code == 409,
                f"매달린 모델 있으면 provider 삭제 409 (got {d1.status_code})",
            )
            # 모델 삭제 후 provider 삭제 204
            check((await c.delete(f"/models/{mid}")).status_code == 204, "모델 삭제 204")
            check(
                (await c.delete(f"/providers/{pid}")).status_code == 204,
                "모델 제거 후 provider 삭제 204",
            )

            # --- 연결 테스트(예외 없이 실패 보고) ---
            print("[probe] 잘못된 base_url 테스트")
            t = (
                await c.post("/providers/test", json={"base_url": "http://127.0.0.1:65535/v1"})
            ).json()
            check(
                t["ok"] is False and t["reachable"] is False, "도달 불가 → ok=False·reachable=False"
            )
    finally:
        await _cleanup()
