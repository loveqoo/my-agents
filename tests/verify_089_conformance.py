...
def _resolve(source, impl):
    ...
    try:
        return chat_mod.resolve_agent_runtime(
            chat_mod.ChatContext(agent_pk=uuid.uuid4(), source=source, impl=impl)
        )
...
def run() -> None:
    ...
    check(
        classify_runtime("external", None) == "non_conforming", "C5 external(A2A) → non_conforming"
    )
    # A2A는 impl 값과 무관하게 non_conforming(원격이 우선 — 인터페이스 미대상).
    check(
        classify_runtime("code", "plan_execute") == "non_conforming",
        "C6 code는 impl 무관 non_conforming",
    )
    check(
        classify_runtime("ui", "does_not_exist") == "config_error", "C7 ui+미등록 키 → config_error"
    )
    check(
        classify_runtime("ui", "bad_stub_089") == "config_error",
        "C8 ui+등록됐으나 부적합 → config_error(갭 봉합)",
    )
    check(
        classify_runtime("ui", "throws_089") == "config_error",
        "C9 ui+생성던짐 → config_error(fail-closed)",
    )
    ...
    check(
        isinstance(pe, PlanExecuteAgent) and isinstance(pe, CustomAgent),
        "G3 적합 키 → 적합 인스턴스",
    )
    check(
        get_agent_impl("bad_stub_089") is None, "G4 등록됐으나 Protocol 부적합 → None(085 갭 봉합)"
    )
    ...
    check(
        isinstance(_resolve("ui", None), DefaultUiAgent),
        "R1 ui+impl없음 → DefaultUiAgent(default 유지)",
    )
    ...
    check(
        _resolve("ui", "does_not_exist") == "RAISE",
        "R5 ui+미등록 키 → AgentConfigError(default 만회 안 함)",
    )
    check(
        _resolve("ui", "bad_stub_089") == "RAISE",
        "R6 ui+부적합 등록 → AgentConfigError(폴백 마스킹 없음)",
    )
...
async def http_checks() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        check(
            r_ui.json().get("conformance") == "conforming",
            "H1 ui+impl없음 → conformance=conforming",
        )
        ...
        check(
            r_bad.status_code == 201,
            f"H2 미해결 impl 저장 허용 201(합의 B) (got {r_bad.status_code})",
        )
        bad_id = r_bad.json()["id"]
        created.append(bad_id)
        check(
            r_bad.json().get("conformance") == "config_error",
            "H2 ui+미해결 impl → conformance=config_error(저장 응답)",
        )
        r_get = await c.get(f"/agents/{bad_id}")
        check(
            r_get.json().get("conformance") == "config_error",
        ...
        async with c.stream(
            "POST",
            f"/agents/{bad_id}/chat",
            json={"messages": [{"role": "user", "content": "안녕하세요"}]},
        ) as resp:
            check(
                resp.status_code == 200,
                f"H3 config_error 채팅 status 200(SSE) (got {resp.status_code})",
            )
        ...
        check(
            err is not None and "does_not_exist_089" not in err,
