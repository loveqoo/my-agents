...
async def main():
    ...
    try:
        ...
        fake_cfg = {
            "llm": {"model_id": "L"},
            "embedder": {"model_id": "E", "api_key": "sk-supersecret12345"},
        }
        ...
        check(
            d["error"] is not None and "실패" in d["error"],
            f"V1a M1 실패가 error로 표면화 (정상 위장 안 함)",
        )
        ...
        check(
            "sk-supersecret" not in buf.getvalue(),
        ...
        check(
            len(res) == 1 and res[0]["text"] == "hit",
            f"V5 2축 중 1축 성공이면 결과 반환(무예외) (got {res})",
        )
        ...
        check(
            "sk-supersecret" not in buf2.getvalue(),
            f"V5c Mem0Backend.search 로그에 비밀 미노출(타입명만) (log={buf2.getvalue().strip()[:80]!r})",
        )

        # ---- V2 M2 재현/수정 (실 mock-embed backend) ----
        # mock cfg 명시 고정(스펙 389 위생, 회고 389): 이 레그는 "해시 임베더라 저유사도→기본
        # threshold서 회상 0"이라는 **mock 전제**의 재현 단언이다. default_mem_cfg는 실모델이
        # 기본값이면(e5) 진짜 유사도가 나와 깨진다 — 환경과 독립하도록 mock 모델을 직접 조립.
        from sqlalchemy import select as _sel
        from sqlalchemy.orm import selectinload as _sl
        from api.mem_config import _build_mem_cfg
        from api.models import ModelConfig as _MC
        from api.db import SessionLocal

        async with SessionLocal() as db:

            async def _mock_model(name: str):
                return (
                    await db.execute(_sel(_MC).where(_MC.name == name).options(_sl(_MC.provider)))
                ).scalar_one_or_none()

            mem_cfg = _build_mem_cfg(await _mock_model("mock-llm"), await _mock_model("mock-embed"))
        ...
        if backend is None or type(backend).__name__ != "Mem0Backend":
            ...
            for t in [
                "The Agent ABCD framework",
                "Autonomy and tool use",
                "Narrow domain agents first",
            ]:
            ...
            check(
                len(default_hits) == 0,
                f"V2a 기본 threshold(0.1)로 회상 0(사용자 증상 재현) (got {len(default_hits)})",
            )
            check(
                len(zero_hits) > 0, f"V2b threshold=0으로 top-k 반환(수정) (got {len(zero_hits)})"
            )
            check(
                d2["stored"] == 3 and len(d2["results"]) > 0,
