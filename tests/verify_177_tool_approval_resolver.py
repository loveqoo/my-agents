...
def main() -> None:
    ...
    check(
        perm("local-tools", "delete_record", tm) == "mcp.local-tools.delete_record",
    ...
    check(
        perm("local-tools", "delete_record", tm_noappr) == "data.delete",
        "R2 approval 없음 → 레거시 data.delete",
    )
    check(
        perm("local-tools", "web_search", tm_noappr) is None, "R2 비위험 도구(레거시 없음) → None"
    )

    # R3 — tools_meta 없음 → 레거시 폴백.
    check(
        perm("local-tools", "delete_record", None) == "data.delete", "R3 tools_meta=None → 레거시"
    )
    check(perm("local-tools", "delete_record", {}) == "data.delete", "R3 tools_meta={} → 레거시")

    # R4 — required=false/누락 → 레거시 폴백(안 걸림).
    check(
        perm("srv", "t", {"t": {"approval": {"required": False}}}) is None,
        "R4 required=false + 레거시 없음 → None",
    )

    # R5 — 임의 신규 도구를 tools_meta로 켜면 하드코딩 없이 게이트.
    check(
        perm("acme", "wire_money", {"wire_money": {"approval": {"required": True}}})
        == "mcp.acme.wire_money",
    ...
    check(
        perm("srv", "t", {"t": {"approval": {"required": True}}}, tp_off) is None,
        "R8 오버라이드 완화(켬→끔)",
    )
    # R8 approver 오버라이드 — 기본 admin, 오버라이드로 self.
    tp_self = {"mcp:srv/t": {"approval": {"approver": "self"}}}
    check(
        appr("srv", "t", {"t": {"approval": {"required": True}}}, tp_self) == "self",
    ...
    check(
        perm(
            ...
            "delete_record",
            None,
            {"mcp:local-tools/delete_record": {"approval": {"required": False}}},
        )
        is None,
    ...
    check(
        appr("srv", "t", {"t": {"approval": {"required": True, "approver": "everyone"}}})
        == "admin",
    ...
    check(
        merged["web_search"].get("approval") == {"required": True},
        "R6 재탐색이 admin approval 이월 보존(게이트 안 지워짐)",
    )
    check(merged["web_search"]["description"] == "새 설명", "R6 설명/파라미터는 탐색 결과로 갱신")
    check(
        perm("srv", "web_search", merged) == "mcp.srv.web_search",
    ...
    check(
        (_tools_meta_from_details(details).get("web_search") or {}).get("approval") is None,
...
def gate_tests() -> None:
    ...
    check(
        gate(relax, admin) is None, "G2 완화 + admin(superuser) → 허용(self-lock: 정당 admin 통과)"
    )
    ...
    apr_relax = {
        "toolPolicy": {"mcp:local-tools/delete_record": {"approval": {"approver": "self"}}}
    }
    ...
    check(
        gate(toctou, nonadmin) == 403,
        "G6 TOCTOU: 비-게이트 도구에 required:false도 비-admin 403(값 기반)",
    )
    check(
        gate({"toolPolicy": {"mcp:acme/t": {"approval": {"required": True}}}}, nonadmin) is None,
