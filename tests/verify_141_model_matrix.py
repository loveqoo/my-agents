...
async def main():
    ...
    try:
        async with async_session() as s:
            await ER.create_case(ds.id, ER.CaseIn(name="기본 문제", input="A/B 테스트에서 중요한 것은?",
                                                   asserts=[{"type": "no_error"}, {"type": "output_nonempty"}]),
                                 session=s, user=sup)
        # M1 게이트
        async with async_session() as s:
            try:
                await ER.start_run(ds.id, ER.RunStartIn(agent_id=agent.id, models=["없는모델XYZ"]),
                                   session=s, user=sup)
                check(False, "M1a 미존재 모델 → 400이어야")
            except HTTPException as e:
                check(e.status_code == 400 and "없는모델XYZ" in e.detail, f"M1a 미존재 모델 400 ({e.detail[:40]})")
        async with async_session() as s:
            try:
                await ER.start_run(ds.id, ER.RunStartIn(agent_id=agent.id,
                                                        models=[default_chat.name, default_chat.name]),
                                   session=s, user=sup)
        ...
        try:
            async with async_session() as s:
                await ER.create_case(ds_rag.id, ER.CaseIn(name="c", input="q", asserts=[{"type": "no_error"}]),
                                     session=s, user=sup)
            async with async_session() as s:
                try:
                    await ER.start_run(ds_rag.id, ER.RunStartIn(collection_id=_uuid.uuid4(),
                                                                models=[default_chat.name]),
                                       session=s, user=sup)
        ...
        gid = _uuid.uuid4()
        async with async_session() as s:
            runs = [EvalRun(dataset_id=ds.id, agent_pk=agent.id, agent_name=agent.name,
                            model_name=m, group_id=gid, status="running", total=1)
        ...
        async with async_session() as s:
            ...
            check(len(grp) == 2 and {g.model_name for g in grp} == {default_chat.name, alt_name},
                  "M2 그룹 2런·model_name 박제")
        await ER._execute_group(specs, ds.id, agent.id, sup)
        async with async_session() as s:
            grp = (await s.execute(select(EvalRun).where(EvalRun.group_id == gid))).scalars().all()
            check(all(g.status == "ok" for g in grp), f"M3a 두 모델 런 모두 ok (got {[g.status for g in grp]})")
            check(all(g.score == 1.0 and g.passed == 1 for g in grp),
                  f"M3b 모델별 성적 정합 (got {[(g.model_name, g.score) for g in grp]})")
            det = await ER.get_run(grp[0].id, session=s, user=sup)
            check(det.model_name is not None and det.group_id == gid, "M3c 성적표에 model_name/group_id")
    finally:
        async with async_session() as s:
            try: await ER.delete_dataset(ds.id, session=s, user=sup)
            except Exception: pass
        async with async_session() as s:
            tmp = (await s.execute(select(ModelConfig).where(ModelConfig.name == alt_name))).scalar_one_or_none()
            if tmp: await s.delete(tmp); await s.commit()
