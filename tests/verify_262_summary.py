...
def main():
    ...
    # S1 격리 접기
    s = _summarize_node_update(
        "요약",
        {
            "messages": [
                rm(1),
                rm(2),
                HumanMessage(content="앞 결과"),
                AIMessage(content="요약 출력"),
            ]
        },
    )
    ...
    check(
        "user: «앞 결과»" in s and "assistant: «요약 출력»" in s,
        f"S1 실제 발화 프리뷰 유지 (got {s})",
    )

    # S2 무회귀 — remove 없는 델타
    s2 = _summarize_node_update(
        "분석", {"messages": [HumanMessage(content="입력"), AIMessage(content="분석 결과")]}
    )
    check(
        "격리" not in s2 and "user: «입력»" in s2 and "assistant: «분석 결과»" in s2,
        f"S2 무회귀(격리 노트 없음) (got {s2})",
    )

    # S3 remove만
    s3 = _summarize_node_update("요약", {"messages": [rm(1), rm(2), rm(3)]})
    check(
        "이전 맥락 3개 정리 (격리)" in s3 and "remove" not in s3 and "+" not in s3,
        f"S3 remove만(노트만, extra 없음) (got {s3})",
    )

    # S4 비밀 불변식(086) — 임의(비안전·비민감) 키의 문자열 값은 원문 미표시, 길이만
    s4 = _summarize_node_update("x", {"custompayload": "verysecretvalue"})
    check(
        "verysecretvalue" not in s4 and "<15자>" in s4,
        f"S4 임의 키 값 원문 미표시(길이만) (got {s4})",
    )
