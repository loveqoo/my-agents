...
check(
    "Message parts",
    a2a_client.extract_text(
        {
            "role": "agent",
            "parts": [{"kind": "text", "text": "안녕"}, {"kind": "text", "text": "하세요"}],
        }
    )
    == "안녕하세요",
)
    ...
    a2a_client.extract_text(
        {
            ...
            "artifacts": [{"parts": [{"kind": "text", "text": "산출"}]}],
        }
    )
    == "산출",
)
...
check(
    ...
    a2a_client.extract_text(
        {
            "kind": "status-update",
            "status": {"message": {"parts": [{"kind": "text", "text": "진행"}]}},
        }
    )
    == "진행",
)
...
check(
    "non-text part 스킵",
    a2a_client.extract_text({"parts": [{"kind": "file", "file": {"uri": "x"}}]}) == "",
)
...
check(
    "error 프레임",
    (
        a2a_client._frame_from_response(
            {"jsonrpc": "2.0", "id": "1", "error": {"code": -32601, "message": "미지원"}}
        )
        or {}
    )
    .get("error", "")
    .startswith("외부 에이전트 오류"),
)
check(
    "text 프레임",
    a2a_client._frame_from_response({"result": {"parts": [{"kind": "text", "text": "응답"}]}})
    == {"text": "응답"},
)
...
check(
    "method send(폴백)",
    a2a_client._jsonrpc_request("x", streaming=False)["method"] == "message/send",
)
...
check(
    "실토큰 → Bearer",
    a2a_client._auth_headers(enc) == {"Authorization": "Bearer super-secret-token"},
)
...
expect_raises(
    "link-local 메타데이터 차단",
    lambda: net_guard.guard_url("http://169.254.169.254/latest/meta-data"),
)
...
expect_raises(
    "IPv6 매핑 ::ffff:127.0.0.1 차단", lambda: net_guard.guard_url("http://[::ffff:127.0.0.1]/x")
)
...
try:
    ...
    frames = asyncio.run(
        _collect(a2a_client.a2a_stream("http://8.8.8.8/x", "tok", "hi", streaming=True))
    )
finally:
...
frames2 = asyncio.run(
    _collect(a2a_client.a2a_stream("http://10.0.0.1/x", None, "hi", streaming=True))
)
