...
async def main():
    ...
    async with SessionLocal() as s:
        ...
        # ── B1 접수 → 배경 완료 ──
        rows = "\n".join(
            json.dumps({"metadata": {"id": i}, "data": f"엔티티 {i} — 배경 인제스트 검증 행."}, ensure_ascii=False)
            for i in range(1, 8)
        )
        doc_out = await RAG.ingest_document(cid, _upload("bg.jsonl", rows.encode(), "application/jsonl"), s, sup)
        check(
        ...
        async with SessionLocal() as s2:
            n = (
                await s2.execute(select(func.count(Chunk.id)).where(Chunk.document_id == doc_out.id))
            ).scalar_one()
            colrow = await s2.get(Collection, cid)
            check(n == 7 and colrow.chunk_count == 7 and colrow.doc_count == 1, f"B1c 청크/집계 실측 ({n}·{colrow.chunk_count}·{colrow.doc_count})")

        # ── B2 형식 위반은 동기 400+행 번호(계약 보존) ──
        bad = rows + '\n{"data": "metadata 없는 행"}'
        try:
            await RAG.ingest_document(cid, _upload("bad.jsonl", bad.encode(), "application/jsonl"), s, sup)
            check(False, "B2 위반 업로드 → 400 (예외 없음)")
        except HTTPException as exc:
            check(exc.status_code == 400 and "8번째 줄" in str(exc.detail), f"B2 동기 400+행 번호 (got {exc.status_code}, {str(exc.detail)[:60]})")
        ...
        try:
            ...
            async with SessionLocal() as s4:
                ...
                vecs = await rag_ingest.embed_texts(
                    emb3.provider.base_url, _crypto.decrypt(emb3.provider.api_key), emb3.model_id, texts5
                )
        finally:
            rag_ingest.EMBED_BATCH = orig_size
            rag_ingest._embed_batch = orig_batch
        check(calls == [2, 2, 1], f"B3a 배치 분할 호출(2,2,1) (got {calls})")
        check(len(vecs) == 5 and all(len(v) == RAG_EMBED_DIMS for v in vecs), "B3b 벡터 5개·차원 정합(순서 보존 병합)")

        # ── B4 캡: 기본 50,000 + env 오버라이드 ──
        check(rag_ingest.ENTITY_MAX_ROWS == 50000 or os.environ.get("RAG_ENTITY_MAX_ROWS"), f"B4a 기본 캡 50,000 (got {rag_ingest.ENTITY_MAX_ROWS})")
        ...
        check("3" == probe.stdout.strip().splitlines()[0] and "RAISED" in probe.stdout, f"B4b env 오버라이드 실측 (out={probe.stdout.strip()[:60]})")
        ...
        async with SessionLocal() as s5:
            ...
            check(
                swept >= 1 and z is not None and z.status == "error" and "재시작" in (z.error or ""),
        ...
        bcol = Collection(
            name=f"{tag}-b7", kind="document", embedding_model_id=emb.id, dims=RAG_EMBED_DIMS,
            chunk_size=1000, chunk_overlap=200, status="ready", owner_id=str(sup.id),
        )
        s.add(bcol)
        await s.flush()
        zdoc2 = Document(collection_id=bcol.id, filename="inflight.md", byte_size=1, status="embedding")
        s.add(zdoc2)
        await s.commit()
        try:
            await RAG.reindex_collection(
                bcol.id, ReindexIn(embedding_model_id=emb.id, chunk_size=500, chunk_overlap=50), s, sup
            )
        ...
        check(page.processing >= 1, f"B8 processing 신호=전역 (page 1건인데 processing={page.processing})")
        ...
        check(probe2.stdout.strip() == "50000 128", f"B4c env 비숫자 → 기본값 폴백 (out={probe2.stdout.strip()!r})")
        ...
        s.add(dcol)
        await s.commit()
        pdoc = await RAG.ingest_document(dcol.id, _upload("broken.pdf", b"not-a-pdf", "application/pdf"), s, sup)
        ...
        check(st6 == "error" and p is not None and (p.error or "") != "", f"B6b 배경 실패 → error+사유 (got {st6}, {p.error[:40] if p and p.error else '?'})")
