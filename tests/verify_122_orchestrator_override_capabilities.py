...
검증(실 DB 통합, 자기 픽스처):
  - B1 조율형 + overrides={"capabilities":[...]} → ctx["capabilities"]가 오버라이드 반영.
...
async def integration() -> None:
    ...
    ctx = await chat._load_context(aid, None, {"capabilities": ["mcp:srv_a", "mcp:srv_b"]}, own=None)
    check(ctx["capabilities"] == ["mcp:srv_a", "mcp:srv_b"],
    ...
    check(ctx["capabilities"] == ["mcp:srv_a"],
    ...
    check(ctx["capabilities"] == [],
          f"B4 빈 리스트 오버라이드 반영(got {ctx['capabilities']!r})")
...
def unit_broker_caller_scoped() -> None:
    ...
    check(len(params) >= 2 and params[0] in ("principal", "user", "caller"),
    ...
    check('"capabilities"' in src and "allowed = {" in src,
