...
CFG = {
    "llm": {"model_id": "gpt-x", "api_key": "sk-SECRET123456", "base_url": "http://h/v1"},
...
class _FakeBackend:
    ...
    def search(
        self, scope, query, limit, **_kw
    ):  # 실 search는 threshold 등 extra kwargs 허용(스펙 158)
        if self._raise:
...
def main() -> None:
    ...
    try:
        ...
        check(
            d["configured"] is False and d["backend_ready"] is False and d["error"],
        ...
        check(
            d["configured"] is True and d["backend_ready"] is False and d["error"],
        ...
        d = memory.recall_diag({"user_id": "u"}, "q", CFG, 4)
        check(
            d["backend_ready"] is True and d["error"] and d["results"] == [],
            f"D 검색 예외 → backend_ready·error·빈결과(got {d})",
        )
        check(
            SECRET not in d["error"], f"D **비밀 마스킹**: error에 api_key 없음(got {d['error']!r})"
        )
        ...
        d = memory.recall_diag({"user_id": "u"}, "q", CFG, 4)
        check(
            d["error"] is None and len(d["results"]) == 1 and d["backend_ready"] is True,
        ...
        for s in [f"api_key={SECRET}", "Authorization: Bearer abc123def456", f"key {SECRET}"]:
            ...
            check(
                SECRET not in masked and "abc123def456" not in masked,
                f"_sanitize 마스킹: {s[:20]!r}",
            )

        # codex 125 H1 — 라벨 뒤 공백형/다른 라벨/hex 값(정규식 백스톱)
        HEX = "0123456789abcdef0123456789abcdef01234567"
        for s in [
            f"401 upstream, api_key: {HEX}",
            f"x-api-key: {HEX}",
            f"OPENAI_API_KEY = {HEX}",
            f"token: {HEX}",
            f'{{"authorization": "{HEX}"}}',
        ]:
            masked = memory._sanitize(s)
        ...
        cfg2 = {
            "llm": {"model_id": "m", "api_key": raw_key},
            "embedder": {"model_id": "e", "api_key": raw_key},
        }
        memory.resolve_backend = lambda cfg: _FakeBackend(
            raise_with=f"upstream said: {raw_key} bad"
        )
        d = memory.recall_diag({"user_id": "u"}, "q", cfg2, 4)
        check(
            raw_key not in (d["error"] or ""), f"H1 정확치환: 실제 api_key 제거(got {d['error']!r})"
        )
    finally:
