...
async def main() -> None:
    ...
    check(
        ...
        f"V1a 성공 검색 raw.hits ≥ 1 (got {raw.get('hits')})",
    )
    check(
        isinstance(raw.get("topScore"), float) and 0 < raw["topScore"] <= 1.0,
    ...
    inv = b.invocations[0]
    check(
        inv.get("hits") == raw.get("hits") and inv.get("topScore") == raw.get("topScore"),
        f"V2b hits/topScore 통과 (got {inv})",
    )
    check(
        inv.get("node", "").startswith("broker_invoke:rag:")
        and "ms" in inv
        and inv.get("cap_id") == "rag:docs-kb",
    ...
    check(
        set(inv.keys()) <= allowed_keys and "args" not in inv,
    ...
    for hd in inv.get("hitsDetail", []):
        check(
            set(hd.keys()) <= {"score", "filename", "collection", "textPreview"}
            and len(str(hd.get("textPreview", ""))) <= 241,
            f"V3b hitsDetail 항목=표시 필드만+프리뷰 캡 (got {set(hd.keys())})",
        )
    check(
        all(not isinstance(v, (list, dict)) or k == "hitsDetail" for k, v in inv.items()),
