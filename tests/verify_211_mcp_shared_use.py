...
def _provision(create: bool) -> None:
    ...
    for email, extra in [(OWNER_EMAIL, ["member"]), (OTHER_EMAIL, ["member"])]:
        ...
        subprocess.run(args, cwd=os.path.join(ROOT, "packages", "api"), check=False,
                       capture_output=True)
...
async def _cleanup() -> None:
    async with SessionLocal() as s:
        await s.execute(delete(AgentVersion).where(AgentVersion.agent_pk.in_(
            select(Agent.id).where(Agent.agent_id == AGENT_ID))))
        await s.execute(delete(AgentVersion).where(AgentVersion.agent_pk.in_(
            select(Agent.id).where(Agent.name == "probe211-agent"))))
...
async def main() -> int:
    ...
    try:
        ...
        r = await owner.post("/mcp-servers", json={
            "name": MCP_NAME, "source": "local", "transport": "http",
            "url": "http://127.0.0.1:8000/_remote/mcp/", "tools": ["echo"],
            "auth": "sekret-211", "published": False,
        })
        check(r.status_code in (200, 201), f"SETUP: owner MCP 등록({r.status_code})")

        # ---- P: published 3입구 — non-custom은 켤 수 없다 ----
        r = await owner.post("/mcp-servers", json={
            "name": MCP_NAME + "-pub", "source": "local", "transport": "http",
            "url": "http://x/", "tools": [], "published": True,
        })
        ...
        full = {"name": base_row["name"], "source": base_row["source"],
                "transport": base_row["transport"], "url": base_row["url"],
                "tools": base_row.get("tools") or [], "published": True}
        ...
        if wf is not None:
            r = await owner.put(f"/mcp-servers/{wf['id']}/publish", json={"published": wf["published"]})
        ...
        r = await other.post(f"/agents/{agent_uuid}/chat",
        ...
        # ---- S: 서빙 게이트 불변 ----
        r = await anon.post("/_served/mcp/web-fetch/", json={"jsonrpc": "2.0", "id": 1, "method": "tools/list"})
        async with SessionLocal() as s:
            wf_row = (await s.execute(select(McpServer).where(McpServer.source == "custom"))).scalars().first()
