...
ok(d11 is True and sorted(t.name for t in t11) == ["call_tool", "search_tools"],
...
ok(r["results"][0]["name"] == "wiki_probe", f"2b '위키백과 검색' 1위=wiki_probe ({r['results'][0]['name']})")
r2 = json.loads(search.func("계산 더하기"))
ok(r2["results"][0]["name"] == "calc_probe", f"2c '계산' 1위=calc_probe ({r2['results'][0]['name']})")
...
out = asyncio.run(call.ainvoke({"name": "wiki_probe", "arguments": json.dumps({"query": "장영실"})}))
...
ok(set(call.args.keys()) == {"name", "arguments"}, f"3e call_tool 스키마 인자=name·arguments ({sorted(call.args.keys())})")
