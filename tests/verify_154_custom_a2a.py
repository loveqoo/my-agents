...
async def main():
    ...
    try:
        ...
        async with async_session() as s:
            ...
            made.append(a.id)
        async with async_session() as s:
            out = await AG.set_agent_visibility(
                made[0], AG.VisibilityIn(public=True), session=s, principal=member
            )
        ...
        async with async_session() as s:
            out = await AG.set_agent_visibility(
                made[0], AG.VisibilityIn(public=False), session=s, principal=admin
            )
            check(
                out.owner_id == str(admin.id) and out.exposed.get("a2a") is False,
        ...
        async with async_session() as s:
            await AG.set_agent_visibility(
                made[0], AG.VisibilityIn(public=True), session=s, principal=admin
            )
        async with async_session() as s:
            try:
                await AG.set_agent_visibility(
                    made[0], AG.VisibilityIn(public=False), session=s, principal="machine"
                )
        ...
        async with async_session() as s:
            ...
            check(
                pe is not None and (pe.config or {}).get("impl") == "pipeline",
                "V2a 시드 커스텀 플로우 존재(ui+impl)",
            )
        ...
        async with httpx.AsyncClient(transport=transport, base_url=VBASE, timeout=120) as c:
            r = await c.get(f"/agents/{pe_id}/.well-known/agent-card.json")
            check(r.status_code == 200, f"V2b 커스텀 플로우 카드 200 (got {r.status_code})")
            r = await c.post(
                f"/agents/{pe_id}/a2a", json=_rpc_body("A2A 루프백 스모크"), headers=headers
            )
            reply = ((r.json().get("result") or {}).get("parts") or [{}])[0].get("text", "")
            check(
                r.status_code == 200 and len(reply) > 0 and "error" not in r.json(),
        ...
        async with httpx.AsyncClient(transport=transport, base_url=VBASE, timeout=120) as c:
            r = await c.post(
                f"/agents/{made[-1]}/a2a", json=_rpc_body("중계 스모크"), headers=headers
            )
            body = r.json()
            reply = ((body.get("result") or {}).get("parts") or [{}])[0].get("text", "")
            check(
                r.status_code == 200 and len(reply) > 0 and "error" not in body,
            ...
            check(
                err.get("code") == -32000 and "루프" in str(err.get("message")),
