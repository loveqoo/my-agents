    ...
    try:
        async with httpx.AsyncClient(
            ...
            check(
                ...
                f"H2 이름='{src['name']}-copy' (got {clone['name']})",
            )
            check(
                "(복사본)" in (clone.get("description") or ""),
                f"H2b 설명에 '(복사본)' (got {clone.get('description')!r})",
            )
            check(clone.get("source") == "ui", f"H3 복제본 source=ui (got {clone.get('source')})")
            # 행위 설정 복사 확인.
            check(
                clone.get("capabilities") == ["rag:x", "mcp:y/z"],
                f"H4 능력 복사 (got {clone.get('capabilities')})",
            )
            check(clone.get("impl") == "orchestrate", f"H4 impl 복사 (got {clone.get('impl')})")
            check(
                clone.get("historyDepth") == 9,
                f"H4 historyDepth 복사 (got {clone.get('historyDepth')})",
            )
            ...
            async with SessionLocal() as s:
                row = (
                    await s.execute(select(Agent).where(Agent.id == uuid.UUID(clone["id"])))
                ).scalar_one()
                cfg = row.config or {}
            check(
                cfg.get("requires_approval") is True,
                f"H4 requires_approval 복사 (got {cfg.get('requires_approval')})",
            )
            check(
                abs((cfg.get("temperature") or 0) - 0.3) < 1e-9,
                f"H4 temperature 복사 (got {cfg.get('temperature')})",
            )
