...
seed 카탈로그 상수를 직접 측정한다(DB 불필요·결정적). 두 축:
  C. 카운트 — 프롬프트 2·컬렉션 3·세션 0·에이전트 5(트림 후 정예).
  R. 참조 무결 + 고아 0 — 시드 에이전트가 가리키는 prompt/vectorTable 자산이 전부 존재하고,
     반대로 모든 프롬프트·컬렉션이 ≥1 에이전트에 참조된다(고아 없음).
...
def main() -> None:
    prompt_names = {p[0] for p in seed.PROMPTS}
    ...
    check("strict-senior-engineer" not in prompt_names, "C strict-senior-engineer 제거됨")
    check("calm-sre" not in prompt_names, "C calm-sre 제거됨")
    check("support-tickets" not in collection_names, "C support-tickets 제거됨")

    # ── R. 시드 에이전트가 참조하는 prompt/vectorTable 수집 ──────────────────
    # ui 에이전트(AGENTS): (aid, name, desc, source, model, prompt, mems, hist, vts, ...)
    ...
    dangling_p = ref_prompts - prompt_names
    dangling_v = ref_vts - collection_names
    check(not dangling_p, f"R1 prompt dangling 0 (발견: {dangling_p})")
    check(not dangling_v, f"R1 vectorTable dangling 0 (발견: {dangling_v})")

    # R2. 고아 0 — 모든 프롬프트·컬렉션이 ≥1 에이전트에 참조됨(정예: 걸리적거리는 미참조 없음).
    orphan_p = prompt_names - ref_prompts
    orphan_v = collection_names - ref_vts
    check(not orphan_p, f"R2 고아 프롬프트 0 (발견: {orphan_p})")
