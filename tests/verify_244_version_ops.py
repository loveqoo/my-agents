...
async def run() -> bool:
    ...
    async with httpx.AsyncClient(
        ...
        try:
            ...
            # O1 — v1 턴 + 피드백 👍
            r = await c.post(
                f"/agents/{a['id']}/chat", json={"messages": [{"role": "user", "content": "안녕"}]}
            )
            ...
            for i, line in enumerate(lines):
                if (
                    line == "event: message_id"
                    and i + 1 < len(lines)
                    and lines[i + 1].startswith("data: ")
                ):
                    mid = json.loads(lines[i + 1][6:]).get("id") or json.loads(
                        lines[i + 1][6:]
                    ).get("messageId")
            if mid is None:
            ...
            fbr = await c.put(
                f"/sessions/{sid}/messages/{mid}/feedback", json={"rating": "up", "reason": ""}
            )
            ...
            ds = (
                await c.post(
                    "/eval/datasets",
                    json={"name": f"ds244-{uuid.uuid4().hex[:6]}", "kind": "agent"},
                )
            ).json()
            ds_id = ds["id"]
            await c.post(
            ...
            v1ops = (ops.get("versions") or {}).get(v1) or {}
            ck(
                v1ops.get("evalRuns") == 1 and v1ops.get("lastScore") == 1.0,
            ...
            await c.put(
                f"/agents/{a['id']}",
                json={"config": {"model": "mock-llm", "prompt": "운영 집계 v2"}},
            )
            ...
            v1ops = (ops.get("versions") or {}).get(v1) or {}
            ck(
                v2ops.get("autoRuns", 0) >= 1 and v2ops.get("evalRuns", 0) >= 1,
                f"O3a v2 자동 회귀 집계 (auto={v2ops.get('autoRuns')} runs={v2ops.get('evalRuns')})",
            )
            ck(
                v1ops.get("evalRuns") == 1 and v1ops.get("up") == 1,
            ...
            ck(
                not ops_b.get("versions") and ops_b.get("unversionedUp") == 0,
            ...
            try:
                ...
                ck(
                    r.status_code == 403,
                    f"O6 공유 에이전트 멤버 ops 403(관리자 전용) (got {r.status_code})",
                )
            finally:
                app.dependency_overrides[current_principal] = lambda: _Super()
        finally:
