...
async def main():
    ...
    check(
        [m.content for m in conv] == ["h1", "a1", "h2", "a2", "cur"], "_to_base_messages 왕복·순서"
    )
    check(
        conv[-1].__class__.__name__ == "HumanMessage" and conv[1].__class__.__name__ == "AIMessage",
    ...
    check(
        [m.content for m in s_def] == ["a2"],
        f"기본(None)=상속 depth 2 → [a2] (got {[m.content for m in s_def]})",
    )
    s4 = await prox(4, node="B")  # conv[-4:][:-1] = [a1,h2,a2]
    check(
        [m.content for m in s4] == ["a1", "h2", "a2"],
        f"depth 4 → [a1,h2,a2] (got {[m.content for m in s4]})",
    )
    s0 = await prox(0, node="C")  # conv[-1:][:-1] = []
    check(s0 == [], "depth 0 → 빈 대화(현재 턴만 시드됨)")
    s_all = await prox(-1, node="D")  # 음수=전체 → conv[:-1]
    check(
        [m.content for m in s_all] == ["h1", "a1", "h2", "a2"],
        f"depth -1=전체 → 현재 제외 전부 (got {[m.content for m in s_all]})",
    )
    check(
        [r["depth"] for r in rec] == [2, 4, 0, -1] and [r["count"] for r in rec] == [1, 3, 0, 4],
    ...
    check(
        [m.content for m in r_all] == ["p1", "p2"],
        f"drop_last=False → 마지막(직전 assistant) 보존 (got {[m.content for m in r_all]})",
    )
    r_1 = await prox_resume(1, node="R2")
    check(
        [m.content for m in r_1] == ["p2"],
        f"drop_last=False depth1 → 마지막 1개(안 버림) (got {[m.content for m in r_1]})",
    )
    ...
    try:
        ...
        ctx = AgentBuildContext(
            ...
            history_window=fake_window,
        )
        await (
            LinearPipelineAgent()
            .build_graph(ctx)
            .ainvoke({"messages": [HumanMessage(content="CUR")]})
        )
        by_prompt = {k: v for k, v in seen}
        # n1(carry, depth6): sys, HIST(d=6), CUR 순 — 대화 슬라이스가 msgs 앞에 주입
        check(
            any("HIST(d=6)" in c for c in by_prompt["P1"]),
            f"엔진: carry n1에 depth6 슬라이스 주입 (got {by_prompt['P1']})",
        )
        p1 = by_prompt["P1"]
        check(p1.index("HIST(d=6)") < p1.index("CUR"), "엔진: 슬라이스가 현재 입력(CUR) 앞에 온다")
        # n2(carry, 미지정): depth=None(상속) 전달
        check(
            any("HIST(d=None)" in c for c in by_prompt["P2"]),
            f"엔진: carry n2 미지정=None(상속) 전달 (got {by_prompt['P2']})",
        )
        # n3(clean): 대화 슬라이스 없음(결정 가)
        check(
            not any(str(c).startswith("HIST") for c in by_prompt["P3"]),
            f"엔진: clean n3엔 대화 슬라이스 없음 (got {by_prompt['P3']})",
        )
        # 프록시 호출: n1(6)·n2(None) 두 번, clean n3은 호출 안 함
        depths_called = [c["depth"] for c in hw_calls]
        check(
            depths_called == [6, None],
            f"엔진: carry 노드만 프록시 호출(clean 제외)·자기 depth 전달 (got {depths_called})",
        )
        ...
        await (
            LinearPipelineAgent()
            .build_graph(ctx3)
            .ainvoke({"messages": [HumanMessage(content="Y")]})
        )
        check(
            not any(str(c).startswith("HIST") for _, msgs in seen for c in msgs),
            "엔진: history_window=None이면 주입 없음(무회귀)",
        )
    finally:
