...
async function deleteMemoryTypeByName(request: APIRequestContext, name: string) {
  const list = await (await request.get(`${API}/memory-types`)).json()
  const m = list.find((x: { name: string; id: string }) => x.name === name)
  if (m) await request.delete(`${API}/memory-types/${m.id}`)
}
...
test('빌딩 블록 — 메모리 타입 등록 → 편집 (015, 생성 후 API 정리)', async ({ page, request }) => {
  const name = uniq('e2e-mem')
  ...
  await page.getByRole('button', { name: '새 항목' }).click()
  const create = page.getByRole('dialog')
  await expect(create.getByText('새 메모리 타입')).toBeVisible()
  await create.getByPlaceholder('예: working_memory').fill(key)
  await create.getByPlaceholder('예: 작업 기억').fill(name)
  await create.getByRole('button', { name: '등록' }).click()

  // 목록에 등장
  await expect(page.getByText(name).first()).toBeVisible({ timeout: 15_000 })

  // 행 클릭 → 편집(과거 죽어 있던 버튼) → 이름 변경 → 저장
  await page.getByText(name).first().click()
  await page.getByRole('button', { name: '편집' }).click()
  const edit = page.getByRole('dialog').filter({ hasText: '메모리 타입 편집' })
  await expect(edit).toBeVisible()
  const renamed = name + '-edited'
  await edit.getByPlaceholder('예: 작업 기억').fill(renamed)
  await edit.getByRole('button', { name: '저장' }).click()

  // 저장 후 이름 반영 확인(API)
  await expect(async () => {
    const list = await (await request.get(`${API}/memory-types`)).json()
    const m = list.find((x: { key: string; name: string }) => x.key === key)
    expect(m?.name).toBe(renamed)
  }).toPass({ timeout: 10_000 })

  await deleteMemoryTypeByName(request, renamed)
})
