...
COLS = [
    {"id": 1, "name": "docs-kb", "embed_base_url": "http://x", "embed_api_key": "k", "embed_model_id": "e"},
    {"id": 2, "name": "team-notes", "embed_base_url": "http://x", "embed_api_key": "k", "embed_model_id": "e"},
]
...
async def main():
    ...
    from api.chat import _MemoryRecallProxy, _rag_tools_for
    ...
    ctx_pipe = {"rag_collections": COLS, "rag_min_scores": None, "nodes_resolved": [{"name": "n"}]}
    tools = _rag_tools_for(ctx_pipe, sink)
    names = [x.name for x in tools]
    check(names == ["search_documents", "search_documents__docs-kb", "search_documents__team-notes"],
          f"P1 노드형=전체 1 + 컬렉션별 N (got {names})")
    ctx_plain = {"rag_collections": COLS, "rag_min_scores": None, "nodes_resolved": None}
    ...
    chat_mod.memory.search = lambda scope, q, cfg: (calls.append((dict(scope), q)) or [{"memory": f"hit:{q}"}])
    chat_mod.memory.format_memory_hits = lambda hits: " / ".join(h["memory"] for h in hits)
    try:
        rec: list = []
        proxy = _MemoryRecallProxy({"user_id": "u1", "run_id": "s1"}, {"cfg": 1}, "사용자 질문", rec)
        ...
        check(rec[0]["cached"] is False and rec[1]["cached"] is True and rec[2]["cached"] is False,
              f"P2 cached 플래그 (got {[r['cached'] for r in rec]})")
        check(r1 == r2 == "hit:사용자 질문" and r3 == "hit:노드 입력", "P2 캐시 반환값 동일·키워드별 상이")
    ...
    try:
        ...
        nodes = [
            {"name": "n1", "prompt": "P1", "model_cfg": MODEL_CFG, "tools": [], "memories": ["단기(세션)"]},
        ...
        check(prox_calls[0]["query"] is None and prox_calls[0]["node"] == "n1", "엔진: user 모드=기본 키워드(None)")
        check(prox_calls[1]["query"] == "OUT[P2]" and prox_calls[1]["node"] == "n3",
    ...
    check(n[0].get("memories") == ["단기(세션)"] and n[0].get("memoryQuery") == "input" and "evil" not in n[0],
