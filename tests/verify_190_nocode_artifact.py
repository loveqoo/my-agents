...
def main() -> None:
    ...
    check(
        normalize_artifact_fields(None) == [] and normalize_artifact_fields({}) == [],
        "N2 빈 입력 → []",
    )

    # 레지스트리 등록 확인
    check(
        type(get_agent_impl("artifact_form")).__name__ == "ConfigDrivenArtifactAgent",
        "N3 artifact_form 등록·적합",
    )
    ...
    graph = ConfigDrivenArtifactAgent().build_graph(
        AgentBuildContext(
            prompt="p", model_cfg=None, checkpointer=InMemorySaver(), impl_config=SPEC
        )
    )
    r = asyncio.run(
        graph.ainvoke({"messages": [{"role": "user", "content": "가입할게"}]}, config=_cfg("t-c"))
    )
    ...
    check(
        any(f["key"] == "plan" and f.get("candidates") == ["무료", "프로"] for f in pf["fields"]),
        "C2 후보 필드 보존",
    )
    # 제출 → artifact(kind=spec.kind)
    r = asyncio.run(
        graph.ainvoke(
            Command(resume={"type": "form", "values": {"plan": "프로", "email": "a@b.c"}}),
            config=_cfg("t-c"),
        )
    )
    art = r.get("artifact")
    check(
        art is not None and art["kind"] == "signup",
        f"C3 artifact kind=spec.kind (got {art and art.get('kind')})",
    )
    check(
        (art or {}).get("data") == {"plan": "프로", "email": "a@b.c"},
        f"C4 데이터 정확 (got {(art or {}).get('data')})",
    )
    ...
    g2 = ConfigDrivenArtifactAgent().build_graph(
        AgentBuildContext(
            prompt="p", model_cfg=None, checkpointer=InMemorySaver(), impl_config={"fields": []}
        )
    )
    r = asyncio.run(
        g2.ainvoke({"messages": [{"role": "user", "content": "hi"}]}, config=_cfg("t-e"))
    )
    check(
        "__interrupt__" not in r
        and r.get("artifact", {}).get("kind") == "form-result"
        and r["artifact"]["data"] == {},
        "E1 빈 명세 → interrupt 없이 빈 산출물(kind 기본)",
    )
    ...
    asyncio.run(
        g3.ainvoke({"messages": [{"role": "user", "content": "고를게"}]}, config=_cfg("t-d"))
    )
    r = asyncio.run(
        g3.ainvoke(Command(resume={"type": "text", "message": "대로 해줘"}), config=_cfg("t-d"))
    )
    # confirm=False(범용은 confirm 안 씀) → 텍스트로 필수 충족 시 바로 확정
    check(
        r.get("artifact", {}).get("data") == {"size": "대"},
        f"D1 텍스트 병합 확정 (got {r.get('artifact', {}).get('data')})",
    )
