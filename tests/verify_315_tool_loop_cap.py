...
def _tool():
    return StructuredTool.from_function(
        func=lambda query="": "검색결과", name=TOOL, description="검색"
    )
...
class _Bound:
    ...
    async def ainvoke(self, messages):
        ...
        return AIMessage(
            content="",
            tool_calls=[{"name": TOOL, "args": {"query": "x"}, "id": "c" + uuid.uuid4().hex[:8]}],
        )
    ...
    async def ainvoke(self, messages):
        if self.rogue_unbound:
            return AIMessage(
                content="",
                tool_calls=[
                    {"name": TOOL, "args": {"query": "x"}, "id": "r" + uuid.uuid4().hex[:8]}
                ],
            )
...
def _run(stop_after=None, rogue_unbound=False):
    ...
    ctx = AgentBuildContext(
        ...
        impl_config={
            "nodes": [
                {"name": "실행", "prompt": "검색해 답하라", "model_cfg": MODEL_CFG, "tools": [TOOL]}
            ]
        },
    )
...
def main():
    ...
    try:
        ...
        check(
            len(tool_msgs) == _TOOL_ROUNDS_CAP,
            f"T1 무한 tool_call 모델도 도구 호출 {len(tool_msgs)}회로 상한({_TOOL_ROUNDS_CAP})",
        )
        last = msgs[-1]
        check(
            not getattr(last, "tool_calls", None) and "최종 답변" in _text_of(last),
        ...
        check(
            len(tool3) == 2 and not getattr(msgs3[-1], "tool_calls", None),
        ...
        check(
            len(tool4) == _TOOL_ROUNDS_CAP and not getattr(msgs4[-1], "tool_calls", None),
