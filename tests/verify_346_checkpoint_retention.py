...
def _chat_frames(agent_id: str, msg: str, session_id: str | None, form: dict | None = None) -> list[str]:
...
async def _cleanup(prefix: str) -> None:
    async with SessionLocal() as s:
        await s.execute(text("delete from checkpoints where thread_id like :p"), {"p": prefix + "%"})
        await s.execute(text("delete from approvals where approval_id like :p"), {"p": prefix + "%"})
        await s.commit()
...
async def main() -> None:
    ...
    check('durability = "exit"' in chat_code.replace("durability='exit'", 'durability = "exit"')
          or 'durability="exit"' in chat_code.replace(" ", ""),
          "R2a chat.py astream이 durability='exit' 전달(코드)")
    check('durability="exit"' in appr_code.replace(" ", ""),
    ...
    async with SessionLocal() as s:
        s.add(Approval(approval_id=f"{tag}-a1", checkpoint=t_appr, status="pending",
                       agent_name="verify346", permission="data.delete"))
        await s.commit()
    r = await checkpoint_retention.release_thread(t_appr)
    check(r == "kept:approval" and await _rows(t_appr) == 1,
          f"R3a 승인 대기 스레드는 관문이 보존 (got {r!r}, rows={await _rows(t_appr)})")
    async with SessionLocal() as s:  # 승인 해소
        await s.execute(text("update approvals set status='approved' where approval_id = :a"),
                        {"a": f"{tag}-a1"})
        await s.commit()
    r = await checkpoint_retention.release_thread(t_appr)
    check(r == "deleted" and await _rows(t_appr) == 0,
    ...
    check(t_fresh not in (dry.get("sample") or []) and await _rows(t_fresh) == 1,
    ...
    async with SessionLocal() as s:
        s.add(Approval(approval_id=f"{tag}-a2", checkpoint=t_stale, status="pending",
                       agent_name="verify346", permission="data.delete"))
        await s.commit()

    dry = await checkpoint_retention.sweep(dry_run=True, ttl_hours=24)
    sample = dry.get("sample") or []
    check(t_orphan in sample and t_stale in sample and dry.get("would_expire", 0) >= 1
    ...
    async with SessionLocal() as s:
        st = (
            await s.execute(text("select status from approvals where approval_id = :a"),
                            {"a": f"{tag}-a2"})
        ).scalar_one()
    check(st == "expired" and await _rows(t_stale) == 0,
    ...
    check(res0.get("status") == "disabled" and await _rows(t_floor) == 1,
    ...
    try:
        ...
        async with SessionLocal() as s:
            n = (await s.execute(
                text("select count(*) from checkpoints where thread_id like :p"), {"p": f"%{fsid}%"}
            )).scalar_one()
        # 1행 = durability="exit"의 실증(기본 async 모드였다면 슈퍼스텝마다 쌓여 3행 이상).
        check(n == 1, f"R9 폼 interrupt: 체크포인트 **1행**만 남고 보존됨(exit 모드 실증) (got {n})")

        _chat_frames(form_agent["id"], "제출", fsid,
    ...
    check(r_paused == "kept:paused" and await _rows(t_paused) == 1,
    ...
    check(r_done == "deleted" and await _rows(t_paused) == 0,
          f"R11b 끝난 턴은 여전히 폐기(보존이 만능이 되지 않게) (got {r_done!r})")

    # 관문 호출부가 실제로 paused를 넘기는가(코드 스캔 — 넘기지 않으면 위 보장은 죽은 코드)
    check("release_thread(thread_id,paused=bool(interrupts))" in chat_code.replace(" ", ""),
