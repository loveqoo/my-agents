...
ok("error" in r and "lang" in r["error"], f"1a wiki_search lang=jp 거부 ({r.get('error', '')[:40]})")
...
def big_handler(request):
    ...
    return httpx.Response(200, content=b"x" * (served_mcp._FETCH_MAX_BYTES + 1),
                          headers={"content-type": "application/json"})

_install(big_handler)
r = json.loads(served_mcp.wiki_search.func("q"))
_restore()
ok("error" in r and "너무 큽니다" in r["error"], f"2 바이트 캡 초과 → error ({r.get('error', '')[:50]})")
...
r = json.loads(served_mcp.wiki_search.func("q"))
_restore()
ok("error" in r and "리다이렉트" in r["error"], f"3 오프호스트 리다이렉트 차단 ({r.get('error', '')[:60]})")
...
ok(r["results"][0]["snippet"] == "알베르트 물리학자", f"4b HTML 태그 제거 ({r['results'][0]['snippet']})")
...
ok({"wiki_search", "wiki_page"} <= served_mcp._SIDE_EFFECT_FREE_TOOLS, "6c 순수 도구 allowlist 등록(부팅 불변식)")
