...
async def main():
    ...
    t = _FakeTool(
        "web_search",
        "웹 검색",
        {"query": {"type": "string"}, "top": {"anyOf": [{"type": "integer"}, {"type": "null"}]}},
        ["query"],
    )
    ...
    meta = BL._tools_meta_from_details(
        [{"name": f"t{i}", "description": "", "params": []} for i in range(150)]
    )
    ...
    check(
        r.ok and len(r.toolsDetail) == 4,
        f"V2a 라이브 탐색 ok+메타 4건 (got ok={r.ok}, {len(r.toolsDetail)})",
    )
    ws = next((d for d in r.toolsDetail if d.name == "web_search"), None)
    check(
        ws is not None
        and "검색" in ws.description
        and any(p.name == "query" and p.required for p in ws.params),
    ...
    async with async_session() as s:
        row = (
            await s.execute(select(McpServer).where(McpServer.name == MOCK_MCP_SERVER_NAME))
        ).scalar_one()
        row_id = row.id
    ...
    async with async_session() as s:
        ...
        check(
            out.tools_meta is not None and "web_search" in out.tools_meta,
            "V3a 재탐색으로 tools_meta 채움",
        )
        check(
            out.enabled_tools == [t for t in before_enabled if t in out.tools],
            "V3b enabled 교집합 보존",
        )
    ...
    async with async_session() as s:
        ...
        check(
            created.tools_meta == {"a": {"description": "도구 a", "params": []}},
            "V4a 생성 시 메타 저장",
        )
    ...
    try:
        async with async_session() as s:
            ...
            check(
                updated.tools_meta == {"a": {"description": "도구 a", "params": []}},
    ...
    for bad, why in [
        ...
        (
            {"a": {"description": "", "params": [{"name": f"p{i}"} for i in range(31)]}},
            "params 31개",
        ),
        ({"a": "문자열"}, "비구조 항목"),
    ...
    ok_in = McpServerIn(
        name="x",
        tools_meta={
            "a": {"description": "d", "params": [{"name": "q", "type": "string", "required": True}]}
        },
    )
    ...
    try:
        ...
        async with async_session() as s:
            out = await BL.rediscover_mcp_server(row_id, session=s, principal=admin)
            check(
                "ghost-tool" not in out.enabled_tools
                and out.enabled_tools == [t for t in orig_enabled if t in out.tools],
