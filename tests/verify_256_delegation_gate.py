...
async def main():
    ...
    mine = _Agent(
        "agt_mine", "내조수", owner_id=other.id, source="ui"
    )  # other 소유(other 관점 '내것')
    ...
    check(
        "agt_secret" in {c.id for c in caps_admin},
        "P1d candidates: admin은 타인 private도 포함(특권)",
    )

    # ---- P1: load 사용 게이트 재검증 ----
    prov_secret = B.AgentProvider(_factory([secret]), principal=other)
    check(
        await prov_secret.load("agt_secret") is None, "P1e load: 타인 private → None(존재 비노출)"
    )
    ...
    check(
        "agt_secret" not in ids_none, "P1g candidates: principal None → private 제외(fail-closed)"
    )

    # ---- P2a: principal None + 로컬 invoke → 정직 에러(크래시 아님) ----
    res = await prov_none.invoke(public, {"text": "hi"})
    check(
        res.error is not None and "principal" in (res.error or ""),
        "P2a invoke: principal None → 정직 에러(무크래시)",
    )
    ...
    check(
        res_b.error is not None and "총량" in (res_b.error or ""),
        "P2b invoke: 예산 초과 → 정직 에러",
    )
