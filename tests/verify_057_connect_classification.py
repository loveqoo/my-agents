...
async def main() -> None:
    ...
    check(
        isinstance(r, dict) and r["manifest"]["model"] == "m" and r["deploy"]["commit"] == "abc",
    ...
    check(
        out.runtime == "my-agents-sdk · Python 2.4.1",
        f"C2 runtime이 deploy에서 (실제={out.runtime})",
    )
    ...
    raised = False
    try:
        await _connect(VBASE + "/_remote/v1/models")  # JSON이지만 카드 아님(name/url 없음)
    except HTTPException as exc:
        raised = exc.status_code == 400
    ...
    check(
        a.commit is not None and len(a.commit) <= 80,
        f"C6 commit 절단(실제 len={len(a.commit or '')})",
    )
    check(a.runtime is not None and len(a.runtime) <= 200, "C6 runtime 절단")
    check(
        a.active_version is not None and len(a.active_version) <= 40,
    ...
    a = _build_code_agent_from_card(
        base_card,
        {"manifest": {}, "deploy": {"versions": [{"version": "v0", "status": "archived"}]}},
        None,
        True,
    )
    check(
    ...
    a = _build_code_agent_from_card(
        ...
        {
            "manifest": {},
            "deploy": {"commit": "c2", "versions": [{"version": "v0", "status": "archived"}]},
        },
        None,
        True,
    )
    ...
    a = _build_code_agent_from_card(
        ...
        {
            "manifest": {},
            "deploy": {"commit": "c3", "versions": ["x", {"status": "active"}, {"version": "  "}]},
        },
        None,
        True,
    )
    check(
        a.active_version == "c3" and all(v.version.strip() for v in a.versions),
        "C6 잡 versions 무시·빈 version 미생성",
    )
    # external 빌더도 길이 하드닝.
    e = _build_external_agent(
        {"name": big, "url": "http://h/" + big, "capabilities": {"streaming": True}}, None, True
    )
    check(
        len(e.name) <= 200 and (e.endpoint is None or len(e.endpoint) <= 400),
    ...
    check(
        req["params"]["message"].get("contextId") == "sess-123",
    ...
    check(
        "follow_redirects=False" in fc_src,
