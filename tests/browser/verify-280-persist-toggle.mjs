...
try {
  ...
  const iPersist = t1.indexOf('대화 저장', iMode)
  check(iMode >= 0 && iPersist > iMode, `① 저장 방식 바로 아래 대화 저장 (idx=${iMode},${iPersist})`)
  check(t1.includes('세션 재개·인스펙터 소급 열람 가능'), `① 관계 설명(ON) 노출`)
  // 토글 off → 설명 전환
  const sw = modal.locator('div', { has: page.getByText('대화 저장', { exact: true }) })
  ...
  // 비영속 전환 → 토글 disabled(279 ② 유지)
  await modal.getByText('비영속', { exact: false }).first().click()
  await page.waitForTimeout(300)
  const swDisabled = await sw.evaluate((el) => el.className.includes('ant-switch-disabled'))
  check(swDisabled, `③ 비영속 → 대화 저장 disabled`)
  ...
  check(!t3.includes('저장·영속') && !t3.includes('대화 저장'), `② step③ 세부에 '대화 저장'/'저장·영속' 부재`)
