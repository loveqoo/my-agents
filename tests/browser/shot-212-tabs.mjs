...
// 4) 평가 — 문제집 탭 안 자식 Tabs(에이전트 평가 / RAG 평가)
await nav('eval')
await page.waitForTimeout(600)
const evalTabs = await page.getByRole('tab').allInnerTexts()
check('평가: kind Tabs(에이전트/RAG)', evalTabs.some((t) => /에이전트 평가/.test(t)) && evalTabs.some((t) => /RAG 평가/.test(t)), JSON.stringify(evalTabs))
check('평가: Segmented 부재', (await page.locator('.ant-segmented').count()) === 0)
await page.screenshot({ path: path.join(OUT, 'eval-agent.png'), fullPage: true })
// RAG 평가로 전환
await page.getByRole('tab', { name: 'RAG 평가' }).first().click()
