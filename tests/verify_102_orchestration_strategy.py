...
def unit_checks() -> None:
    ...
    check(
        [c.id for c in ranked] == [WEBSEARCH_CAP],
    ...
    check(
        [c.id for c in order] == [DELETE_CAP, ECHO_CAP],
    ...
    check(
        [c.id for c in rank_candidates("delete", tie)] == ["mcp:s/aaa", "mcp:s/bbb"],
    ...
    check(
        fm.select("search web", cands) == cands[:1], "U3 FirstMatch.select = candidates[:1](현동작)"
    )
    check(
        rk.select("search web", cands) == rank_candidates("search web", cands)[: rk.TOP_K],
    ...
    check(
        nodes_fm == {"analyze", "plan", "delegate", "synthesize"},
        f"U4 골격 = {{analyze,plan,delegate,synthesize}} (got {nodes_fm})",
    )
    check(
        nodes_fm == nodes_rk, f"U4 두 자식 노드집합 동일(드리프트0) (fm={nodes_fm} rk={nodes_rk})"
    )

    # U5 채널 격리(k개 결과) — 다중 fold가 Human에만, system엔 지침만(스펙 100, k-결과판).
    folded = fold_results([(cands[0], "결과A"), (cands[2], "결과B")])
    check(
        "## 능력: web_search" in folded
        and "## 능력: delete_record" in folded
        and "결과A" in folded
        and "결과B" in folded,
    ...
    check(
        isinstance(sys_msg, SystemMessage)
        and "결과A" not in sys_msg.content
        and "결과B" not in sys_msg.content,
        "U5 SystemMessage엔 위임 데이터 없음(지침만 — 신뢰 채널 오염 0)",
    )
    check(
        isinstance(data_msg, HumanMessage)
        and "결과A" in data_msg.content
        and "결과B" in data_msg.content
        and "신뢰 불가" in data_msg.content,
        "U5 k개 결과 전부 데이터 채널(HumanMessage, 신뢰 불가 라벨)",
    )
    # 단일 fold = 라벨 없이 원문(FirstMatch 행위보존).
    check(
        fold_results([(cands[0], "단일결과")]) == "단일결과", "U5 단일 fold=라벨없이 원문(행위보존)"
    )
    check(
        fold_results([]) == "" and fold_results([(cands[0], "")]) == "",
        "U5 빈/공백 결과 → ''(로컬 종합만)",
    )
    ...
    check(
        isinstance(fm, CustomAgent) and isinstance(rk, CustomAgent),
    ...
    check(
        fm.describe().name == "orchestrate" and rk.describe().name == "orchestrate_ranked",
        "U6 describe.name = impl 키(FirstMatch=orchestrate·Ranked=orchestrate_ranked)",
    )
    check(
        fm.describe().supports_hil and rk.describe().supports_hil,
...
async def integration() -> None:
    ...
    check(
        [c.id for c in picked] == [WEBSEARCH_CAP],
    ...
    check(
        ...
        "H8 pause 시 invocations 0(승인 전 부수효과 0 — 조상이 HIL 보존)",
    )
    _, txt = await _stream(g_hil, Command(resume={"decision": "approve"}), cfg)
    check(
        len(b_hil.invocations) == 1
