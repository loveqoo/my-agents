...
async def main() -> None:
    ...
    check(
        rp.startswith("[문서 검색 결과") and "유사도" in rp,
        f"V1a resultPreview=검색 스니펫 (앞부분 {rp[:40]!r})",
    )
    ...
    class FakeProv:
        ...
        async def invoke(self, row, args):
            ...
            return InvokeResult(
                text="응답 api_key: sk-LEAKME123456 끝", trust="untrusted", error=None, raw={}
            )
    ...
    check(
        proj[0].get("resultPreview") == "본문"
        and "args" not in proj[0]
        and proj[0].get("node") == "n",
        f"V3 resultPreview·node 통과·args 차단 (got {proj[0]})",
    )

    # V4 — 노드 요약 안전 키
    s1 = runtime._summarize_node_update(
        "analyze", {"query": "옵시디언 노트에서 A/B 테스트 정리해줘"}
    )
    ...
    check(
        s3 is not None and "sk-SECRET-VALUE-999999" not in s3 and "자>" in s3,
    ...
    try:
        ...
        check(
            "유사도 0.910" in calls[0]["result"],
            f"V5 직접 RAG result=스니펫 (got {calls[0]['result'][:60]!r})",
        )
    ...
    sm = _build_sent_messages(
        "프롬프트 api_key: sk-PROMPT-LEAK-99999",
        [{"role": "user", "content": f"m{i}"} for i in range(50)],
    )
    check(
        "sk-PROMPT-LEAK-99999" not in sm[0]["content"] and "«secret»" in sm[0]["content"],
        f"V6a system(프롬프트) 비밀 마스킹 (got {sm[0]['content'][:40]!r})",
    )
    check(
        len(sm) == 1 + 1 + 30 and sm[1]["role"] == "notice" and "생략" in sm[1]["content"],
    ...
    check(
        s7 is not None and "abcdef123456" not in s7 and "«secret»" in s7,
