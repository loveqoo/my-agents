...
async def _dump_state() -> dict:
    ...
    async with SessionLocal() as s:
        providers = [
            {"name": r[0], "kind": r[1]}
            for r in (await s.execute(text("SELECT name, kind FROM providers ORDER BY name"))).all()
        ]
        models = [
            {"name": r[0], "model_id": r[1], "kind": r[2], "is_default": r[3]}
            for r in (
                await s.execute(
                    text("SELECT name, model_id, kind, is_default FROM models ORDER BY kind, name")
                )
            ).all()
        ]
        agents = [
            {"name": r[0], "model": r[1]}
            for r in (await s.execute(text("SELECT name, model FROM agents ORDER BY name"))).all()
        ]
                ...
                for r in (
                    await s.execute(
                        text(
                            ...
                            f"LEFT JOIN models m ON m.id = c.{emb_col} ORDER BY c.name"
                        )
                    )
                ).all()
            ]
        ...
        dangling = sorted(
            {a["model"] for a in agents if a["model"] and a["model"] not in model_names}
        )
        return {
            "providers": providers,
            "models": models,
            ...
            "dangling_agent_models": dangling,
        }
