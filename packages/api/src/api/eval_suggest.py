...
출제 재료 = 대상 에이전트의 구성(_load_context 재사용: 페르소나+RAG 능력). 능력별 전략:
- RAG형: 그 컬렉션 청크 기반 질문(142 출제기 재사용) + 기준 trace_has "rag:"(도구를 실제로
  쓰는가 — agent 런의 관측 union 축).
- 페르소나형: "이 역할이라면 받을 법한 질문" + 기준 llm_judge(역할 충실 — 139 재사용).
...
async def _gen_persona_questions(persona: str, n: int, llm_cfg: dict) -> list[str]:
    """페르소나 기반 질문 N개를 **한 호출**로(1건×N회는 중복·형식 이탈로 수율 저조 — verify_143
