"""유저 장기기억 통합 잡(스펙 039) — jobs.py에서 분할(스펙 395 P6).

_consolidate(구 CC 13)는 LLM 호출(_call_consolidation_llm)과 줄 파싱(_parse_consolidation_lines)
순수 분해, consolidate_user_memories(구 CC 11)는 실행 루프(_run_consolidation) 추출 — **saga
경계는 무변경**(①스냅샷 commit → ②통합본 add(실패 시 삭제 금지·degraded) → ③박제 mem_id만 삭제).
monkeypatch 소비자(verify_039·357)는 이 모듈의 `_consolidate`·`default_mem_cfg`를 패치한다.
파사드는 jobs.py(재수출 계약).
"""
...
from ..models import MemorySnapshot, User
...
def _call_consolidation_llm(texts: list[str], llm: dict) -> str | None:
    """mem_cfg["llm"](openai-호환) 동기 호출 — 실패는 None(호출측이 []로 접어 삭제 차단)."""
    try:
...
def _parse_consolidation_lines(content: str) -> list[str]:
    """LLM 응답 → 줄 단위 사실 리스트(번호/불릿 머리 제거·소문자 기준 dedupe)."""
    facts: list[str] = []
    seen: set[str] = set()
...
def _consolidate(texts: list[str], mem_cfg: dict | None) -> list[str]:
    ...
    if not texts or not llm.get("base_url") or not llm.get("model_id"):
        return []
    content = _call_consolidation_llm(texts, llm)
    if not content:
        return []
    return _parse_consolidation_lines(content)
...
async def _run_consolidation(
    candidates: list, mem_cfg: dict, run_id: uuid.UUID | None
) -> tuple[list[dict], list[str], int, int]:
    """실행 루프(스펙 395 분해) — 반환 (성공 목록, 적재실패 유저, before 합, after 합)."""
    ...
    return consolidated, failed_users, total_before, total_after
...
async def consolidate_user_memories(*, dry_run: bool, run_id: uuid.UUID | None = None) -> dict:
    ...
    consolidated, failed_users, total_before, total_after = await _run_consolidation(
        candidates, mem_cfg, run_id
    )
