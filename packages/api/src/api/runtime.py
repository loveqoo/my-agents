...
def _wrap_agent_tool(broker: Any, cap: Any) -> StructuredTool:
    ...
    async def _delegate(text: str = "") -> str:
        res = await broker.invoke(cap_id, {"text": text})
...
# rag_runtime의 runtime-헬퍼 의존은 함수 내 지연 import라 이 모듈-레벨 재수출이 순환을 만들지 않는다.
from .rag_runtime import (  # noqa: E402,F401
    RagSearchError,
    _annotate_cutoffs,
    _hits_detail,
    _norm_min_scores,
