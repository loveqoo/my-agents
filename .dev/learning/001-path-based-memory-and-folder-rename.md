...
- Claude Code의 영속 메모리는 **프로젝트 절대경로**를 키로 저장된다.
  (예: `~/.claude/projects/-Users-anthony-Repository-github-{{GITHUB_ORG}}-my-agent/memory/`)
...
- 저장소는 git 원격(`{{GITHUB_ORG}}/my-agents`)에도 푸시되어 이중으로 보존된다.

## 진행한 결정/사실
- 로컬 폴더명 `my-agent`(단수)와 원격 `{{GITHUB_ORG}}/my-agents`(복수)가 불일치했음.
