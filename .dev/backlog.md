...
- [후보 **스펙 299**] `end_session` write-scope 정합(codex 298 인접발견): mutating route가 read-scope(`own_scope(·,"sessions","read")`)라 sessions:read 운영자 부여 시 타인 세션 종료 가능(스펙209 F1이 feedback엔 `_own_scope_write` 넣었으나 end 누락). **회귀 아님**(pristine HEAD도 동일, 298 시맨틱 보존). 단순 스왑 불가—end는 machine 토큰(str) 허용, `_own_scope_write`는 `str(principal.id)`라 machine서 깨짐. machine-aware write-scope 설계+verifier(machine 종료가능·read운영자 불가·owner 자기것·superuser 전체) 필요.
- [후보] dedup 후속(census): C(eval llm_cfg dict 빌더 3중)·D(eval `_execute_*` 배경작업 스캐폴딩 3~4중, 최대절감~60-100줄이나 `_active_jobs` 락 lifecycle·사용자 노출 문구 위험→async 컨텍스트매니저+verify_142/143/195/209 필수).
