...
## 후보
- ✅**외부 의존성 운영 가드 = 스펙 430 완료**(2026-07-29, 회고 353): kpas 비교(구남님 공유) 7항목 중 유일한 실측 빈틈 #2 채택 — net_policy.py 대장(모델 180/0·평가 60/1·A2A/MCP 흡수 등 9키)+서킷브레이커(3연속→30s, host 격리, 구남님이 "과설계 OUT" 기각을 교정)+GET /admin/net-policies. codex P1 5 봉합. verify_430 22/22. **kpas 나머지 기각 근거**(재론 비용 0): #6 롤백=평가 게이트(372) 있음·카나리는 k8s 영역, #3 권한·감사=브로커 RBAC(112)·감사컬럼(343) 있음·org/팀 테넌시는 회사 층, #5 관측=OTEL(328) 있음·백엔드/대시보드는 회사 숙제, #7 회귀셋=그물(414)·codex 있음·부하테스트는 후속 후보, #1 스키마 템플릿·#4 장애 플레이북=개인 도구엔 금칠(후순위).
- ✅**모델 설정 UI 재설계 = 스펙 412 완료**(2026-07-20, 회고 340): 능력(사실=Switch+Tag 선언)/파라미터(값=Slider+InputNumber)를 다른 시각언어·블록 분리(모델폼), 오버라이드 화면=프리셋 Segmented+상속 placeholder+오버라이드 배지+되돌리기+게이트 disabled/사유. 서브에이전트 디자인 조사 근거(외부사례+NN/G+VS Code+antd). 모델폼 브라우저 확인+tsc0. **OUT**: 노드형 thinking 다단계(스펙 413)·effort형 thinking·파라미터 배타 안내·사용자 프리셋 저장.
- ✅**노드형 thinking 분리 = 스펙 413 완료**(2026-07-20, 회고 341): reasoning 프레임에 노드 태그(langgraph_node)+FE 노드별 누적(reasoningSteps)→노드형 ThoughtChain(노드별 스텝)·직접형 단일 Think 무회귀. 판정을 데이터에서(스텝 수). 축 분리 유지. verify_413 5/5(결정적, fake reasoning 모델 — 라이브는 실모델 확률성으로 불안정). **OUT**: 노드별 출력(content) 스텝·조율형 위임 사고·status 정밀 매핑. 원 항목: 노드형(파이프라인)은 노드마다 사고가 나올 수 있는데, 현재 SSE reasoning 프레임에 노드 태그가 없고 FE onReasoning이 단일 reasoning 필드에 통째 이어붙여 노드 A+B 사고가 한 패널에 뭉개짐(스펙 410 OUT). 제대로=백엔드 reasoning/content 프레임에 노드 식별자(observed [{node,ms}] 축과 맞물림)+FE ThoughtChain(@ant-design/x, 노드별 스텝: 이름+사고 접이+출력). 직접형은 단일 Think 유지. 412(설정 UI) 끝낸 뒤.
...
- ✅**모델 튜닝 파라미터 N-확장 단일화 = 스펙 411 완료**(2026-07-20, 회고 339): 409 서술자를 숫자까지 일반화(temperature·top_p·max_tokens·repetition_penalty), temperature 특례 흡수(077 은퇴)·wire별 배선(top/extra_body)·캐스케이드 4층·UI kind별 렌더 한 컴포넌트. codex 2패스(세션>저장 순서·노드형 세션표면·이중거처 단일화·유한수). verify_411 24/24+브라우저. **OUT**: presence/frequency_penalty(서술자 한 줄로 후속)·파라미터 프리셋·모델별 지원 자동탐지. 원 항목: 409가 stream/thinking 2개만·temperature는 별도 특례라 3중 불균일. 모델마다 튜닝 메타값(temperature·repetition_penalty·top_p·max_tokens 등)을 서술자로 선언→모델 폼 기본값·에이전트/노드/세션 오버라이드·ChatOpenAI 관통을 하나로. 지금 build_chat_openai는 temperature만 명시배선(나머지 model.params에 있어도 미전달). CapabilitySettings를 숫자/enum까지 일반화. temperature 흡수 여부 결정 필요(스펙 077 깊은 배선).
- ✅**그물 fresh-DB-per-run 격리 하네스 = 스펙 414 완료(2026-07-20, 회고 342)**: db 그룹을 run당 virgin DB 1개(_dbharness.fresh_db, 부트스트랩 1회 상각)로 격리 → 공유 dev DB 플레이크 소멸(3회 연속 91/0/0, **api 가동중** = 동시활동 격리 실증). verify_124도 그린(공유 DB가 근본이었음 — 실모델 비결정 아님). 하네스가 노출한 결정적 red 4건(ambient 의존 verify_050/148/330/352)은 부트스트랩 init_authz 미러+헬퍼 재수출+테스트 자기완결 수리로 해결. 규율([[net-runs-are-exclusive]])을 구조로 승격.
- ✅**첨부 후속 경화 = 스펙 415 완료(2026-07-20, 회고 343)**: ①전역 body limit(2MB/업로드 6MB, ASGI send-재작성)+업로드 rate limit(분당 10회) ②턴 예산(messages 200·현재턴 5만자, 마지막 user 검사) ③첨부 유래 턴(히스토리 재생·A2A 서빙 포함) 부수효과 도구 승인 강제(그래프-tools+broker, 읽기전용 면제). codex 적대 P1 3(A2A 재생 우회가 최대 구멍 — [[installed-guard-isnt-covering-guard]])·P2 4 봉합, verify_415 25/25. **곁가지 잔여**: ⓐ턴 전체 컨텍스트 예산의 모델별 자동 조절(히스토리 depth 상한 유지, 별도 스펙) ⓑ레거시 pending Approval 첫 턴 미영속분(유한 마이그레이션 구멍, 스펙 415 명시) ⓒrate limit 다중 인스턴스 공유(k8s 백로그와 합류).
- ✅**suite 진행 노출 = 스펙 431 완료**(2026-07-29, 회고 354): ProgressWriter(/tmp/suite-progress.json 원자 갱신·실패 무해)+씨앗 그물 per-test [n/N] 스트리밍(직렬 침묵 소멸)+배터리 [n/N]. 실측=tail 실시간·done 증가·종료 일치. 원 항목: **suite 진행 노출(구남님 제안, 2026-07-19)**: 장시간 스위트가 밖에서 안 보임 — 1차 조치=호출 습관(tail 캡처 금지, 전체 로그 파일로 흘려 tail -f 가능하게, 이번 세션부터 적용). 2차 후보=run.py가 진행 파일(예 /tmp/suite-progress.json: 완료 n/51·현재 키·경과)을 갱신하거나 heartbeat 줄 주기 출력 — 소형.
- ✅**배터리-한정 3인조 플레이크(137runner·140·141) = 스펙 414로 해소**(2026-07-20, 회고 342): 유력 근인(db층 라이브 dev DB 공유·동시활동 오염)이 정확했다 — run당 virgin DB 격리로 세 건 모두 그린 안정화(3회 연속). 규율(배타 구간)을 하네스 구조로 대체. **곁가지 관찰**: make test-all의 http 층은 여전히 공유 DB(스펙 385/390 격리 있으나 별개 축) — 재발 시 그때.
- ✅**능력→설정 파생 단일화 = 스펙 409 완료**(2026-07-20, 회고 337): 408 검토서 구남님이 4결함 지적(모델폼 스트리밍·thinking 중복·설정마다 재구현·플그 오버라이드 부재·노드형 미설계)+"복리 안 취하고 바로 작업". 뿌리=이미 있던 범용 캐스케이드(temperature/OverridePanel) 확장 않고 옆에 하드코딩 통로 신설. 수정=서술자 단일정본(capabilities.py)서 화이트리스트·유효값·4화면 파생(설정=한 줄)·모델폼 능력당 한 블록·단일 CapabilitySettings 컴포넌트·플그 세션 오버라이드 실통로·노드별 오버라이드. 층=모델→에이전트→노드→세션. codex 2패스 P1 2건(세션>노드 순서·bool("false") 게이트 우회) 봉합+P2 3건. verify_409 21/21+e2e(중복 소멸 화면). **이관 주의**: thinking 이제 능력 게이트라 기존 모델 thinking 꺼진 채 시작(관리자가 켜야). **OUT**: max_tokens 등 비-능력 설정(cap=None 확장)·비전 요청게이트·능력 자동탐지. [[extend-generic-not-parallel-hardcode]][[whole-fix-over-minimal-patch]]
- ✅**모델 능력 관리 = 스펙 408 완료**(2026-07-19, 회고 336): 2층(능력=모델 선언 불가침/사용=부분집합)+3층 설정 캐스케이드(모델 params→에이전트 modelParams→세션, temperature 제외=단일거처). capabilities JSONB+disable_streaming 안전실행+단건 배지(노드 없는 직접경로만). codex P1×3(능력 PUT시 그래프캐시 회전·노드 명시모델 modelParams 관통·세션 층)+P2(혼재모드 침묵) 반영. verify_408 21/21+SUITE_OK. **OUT**: 비전 게이트·능력 자동탐지(probe)·max_tokens 등 화이트리스트 확장·A2A 서빙 카드 streaming 선언 제어. 발단=구남님 "스트리밍 처리 관리+대화 표시", MLX 가이드 실측(스트리밍 기본 OFF·thinking 기본 ON).
- ✅**SSE 이탈 커넥션 풀 오염 = 스펙 403 완료**(2026-07-19): pool_pre_ping+체크포인터 check+shielded_release 관문(강참조)+A2A 서빙 관문 소급(codex P1), 공격형 verify_403 18/18(봉합 전 200/500 교차 실증). 원 항목: **SSE 이탈 커넥션 풀 오염 봉합(스펙 402 실구멍 보고, 2026-07-19)**: 클라이언트가 chat SSE를 중도 이탈(탭 닫기·모바일)하면 ASGI 취소가 in-flight asyncpg/psycopg 커넥션을 오염 → 공유 풀 poison → 후속 요청 인증 쿼리 비결정 500(068 하네스가 우연 재현, deep-reasoner+codex 실구멍 확인). 봉합 스케치: ①db.py pool_pre_ping=True ②체크포인터 psycopg 풀 checkout 검증 ③event_stream finally의 release_thread를 shield+timeout ④early-abort 회귀 테스트. 근거=.dev/reviews/402-quarantine/codex.out.
...
- ✅**suite 2건 복구 — 8045 API key = 해소 확인(2026-07-20 재측정)**: 구남님이 provider에 키를 이미 설정(MLX key_len=164) → pipeline-override-temperature·pipeline-memory-recall 2/2 통과 실측. **교훈**: 사용자 조치 대기 항목은 질문 전에 **먼저 재측정** — 구남님이 조치한 뒤 백로그가 화석으로 남아 같은 질문이 반복됐다(2026-07-20 지적).
- ✅**저장 config 미등록 모델 폴백 재설계 = 스펙 428 완료**(2026-07-24, 회고 351): 원인 두 입구 봉함(P1① rename 참조가드·P1② 저장 검증) + P2③ resolve 거절. codex 적대 실버그2(공백 드리프트·clone 우회) 봉합. verify_428 12/12. 원 항목: **저장 config 미등록 모델의 조용한 폴백 재설계(스펙 401 OUT)**: 오버라이드는 실명 거절로 정리됐으나 저장 config에 적힌 미등록 모델은 여전히 조용히 기본 폴백(learning 092의 "선언-but-broken=거절" 축과 어긋남 — 기존 docstring 백로그). 거절로 돌릴지, 폴백 유지+UI 경고로 갈지 결정 필요.
- ✅**101/102 인터럽트 조사 = 스펙 391 완료(P0 배제)**(2026-07-18, 회고 396): 승인 게이트 정상(전송0→승인→1회·거부0·실HTTP 왕복 그린) — 실패는 177 P2 스키마 화석(permission 구조화·판정=approver 필드)의 값 단언. 현행화로 101·102 virgin 그린·격리 -2(노후 잔여 11건).
- ✅**격리 하네스 P2 = 스펙 390 완료**(2026-07-18, 회고 395): 위 하네스 항목 참조. fast-worker 첫 위임 성공(위임 갭 교정 실증).
- ✅**테스트 위생 묶음 = 스펙 389 완료**(2026-07-17, 회고 394): 격리 −3(158 mock-cfg 독립화·059/345 virgin 마커 승격) · 068 재격리(해제 실측 실패 — asyncpg 커넥션 수명 조사 유지) · 268 죽은 이름 정리·020 오판 정정(마이그레이션 입력 픽스처) · 리퍼 블록(모델/프로바이더/컬렉션) 확장 · suite 픽스처 회상 실증 자가검사 · 리비전 중복 의심=파싱 오경보 확정.
- ✅**dev 초기화 장치화 = scripts/reset_dev.py**(2026-07-17, 구남님 "원칙 잘 지켜달라" 당부의 즉답 — 회고→장치 승격 갭 폐쇄): 정지(pkill 패턴만—vite 함정 내장 회피)→DB 초기화→재기동→실모델(MLX) 복원+기본값→최종 검증(시드·죽은 행·vite) 일괄. 무인자=계획 출력, --yes 필수(안전 기본값). make reset-dev[-yes]. 위임 미활용 갭은 메모리(use-the-six-step-loop-fully)에 박아 차기 세션 강제.
- ✅**A2A 서빙 v2 = 스펙 388 완주(P1+P2)**(2026-07-17, 회고 392·393): P1 세션 연속성+영속 — verify_388 10/10·suite 51/51·codex 보안결함 0. 결정: 영속·세션화면 표시 승인, P2 재개 입구=(a) A2A 순수. P2 승인 브리지(input-required Task·A2A 순수 재개·codex P1 결재인가 우회 봉합) 완료. **잔여 관찰**: ⓐ재개 턴 자동 기억 저장 빚(기존) ⓑ서빙 relay(code) 경로 세션/승인 미지원(정직한 경계 유지) ⓒA2A tasks/get 폴링·푸시 표준 표면(필요 시). — 원 항목:(2026-07-16 접수, 구남님 "백로그에 올려줘" — 387 후속 문답에서 도출). **동기**: 외부 연동의 주력이 A2A 표준이라면, 지금 A2A JSON-RPC 입구는 유저 특정(metadata.userId, 387)+기억(user 축)까지만 — 세션·승인은 채팅 API(머신 Bearer)로 우회해야 한다. **현재 상태(조사 완료)**: 서빙(`stream_local_reply`)은 v1 무상태(스펙 061 §6 — persist·세션·HIL 미적용, 호출당 단일 메시지). 우리 a2a_client는 contextId를 **보내지만**(스펙 057, chat_stream.py:87) 우리 서버는 **소비 안 함**(chat_stream.py:205 "맥락은 호출측 책임"). 승인 필요 도구는 checkpointer=None이라 fail-closed(안전 거부). **설계 조각**: ①contextId→세션 매핑(수신 시 세션 resume/생성 — RBAC 체크리스트 "외부 프로토콜 입구" 항목 정면 적용: contextId 추측으로 타인 세션 잡기 방지 필요, userId+contextId 바인딩 검증) ②승인 브리지=A2A 표준 input-required 태스크 상태로 승인 대기를 표현(우리 approvals와 왕복) ③재개 턴 자동 기억 저장 빚(chat_approval "user_id 부재로 생략" 주석)도 함께. 비교표·근거는 387 후속 문답(2026-07-16) 참조.
...
- ✅**INDEX 키워드 배열 압축 = 컴팩션 완료**(2026-07-19, 커밋 0f30786): 서사 후크화+키워드 캡 8(사용자 승인) — 총자수 −12.6%, 불변식(항목수·참조) 기계 검증. 원 항목: **INDEX 키워드 배열 압축(2026-07-15 재압축 후속)**: 재압축으로 후크는 최소화됐으나 능력 브로커 시리즈(spec/retrospect 069~105 등)는 **키워드 배열 자체가 200~540자**(키워드 15~22개)라 줄이 못 줄었다. 키워드는 recall 앵커라 이번엔 "절대 보존"으로 뒀지만, 20개는 과할 수 있음 — 항목당 핵심 5~8개로 추릴지 판단 필요(단, recall 적중률 저하 위험이 있어 측정 동반 필요, 지금은 급하지 않음).
- [ ] **자원 감사 캠페인 후속(스펙 346~352 OUT, 2026-07-15)**: ~~①살아있는 유저 기억·메시지의 상한~~ **→ 스펙 429 완료**(2026-07-28, 회고 352): 기억 1000/유저(add 관문 축출·완전 수렴)·메시지 180일 기본ON(session-cleanup 활성). codex 실결함2 봉합. verify_429 10/10. ~~②RAG 인제스트 메모리~~ **→ 스펙 432 완료**(2026-07-30, 회고 355): 실측(21k 청크 Δ+1.75GB·잔류 +0.98GB — 추정보다 악화)→배치 흘려보내기+세마포어(2)로 피크 9배↓·잔류 65배↓. codex P0(재인덱싱 완주 창→재료 관문) 봉합. ~~후속 씨앗: 재인덱싱·편집 경로의 전량 임베딩~~ **→ 스펙 433 완료**(2026-07-30, 회고 356): 재인덱싱 12.5배↓·편집 5.9배↓, codex P1(동시편집 LWW) 봉합. 그 스펙 P2(취소 시 reindexing 고착)도 **→ 스펙 434 완료**(회고 357: except BaseException+새 세션 shield 해제·취소 이력 발자국, TTL 스윕은 살아있는 잠금 탈취 위험으로 기각) ~~③배치 트리거의 dry-run 날카로운 모서리~~ **→ 스펙 383 완료(2026-07-16)**: body 수용+안전 기본값(dry-run)+StrictBool(codex P1). verify_383 10/10.
...
- ✅**기존 게이트 드리프트 2건 = 스펙 400 재활로 해결**(2026-07-19): 034=스펙 324 버킷 축소(live=active만) 기대치 재작성·098=죽은 error 버킷 AND 검증을 산 버킷으로 이전, 둘 다 그물 복귀. 원 항목: (2026-07-14 확증) verify_098_session_search P?("plainagent+error → S2만"에 sess_v098_MARKERID_1이 섞임) · verify_034_session_pagination(KeyError 'awaiting' — counts 버킷 4종 기대치가 스키마/응답과 어긋남). 격리 DB에서도 재현 → 검증기 노후. 336류의 "게이트 드리프트" 묶음.
...
- [ ] **브라우저 verify 실버그 의심 3건 조사(스펙 438 triage 산출, 2026-08-04)**: ①verify-312-reindex "재인덱싱 이력 0건"(**434/436 이력 변경 접점 — 우선**) ②verify-344-audit-ui 감사 actor 표시 U1/U2 ③verify-inspector-matrix 회상 trace 미표시(스펙 362 축). 각각 솔로 재현→앱 결함/스크립트 노후 판정.
- [ ] **브라우저 verify 격리 재활 캠페인(스펙 438 후속)**: 격리 43건을 군집별로(계정픽스처 7·스펙370 3·UI현행화 24·환경 2·크래시 4·437샘플 1) 재활 — 러너 `KNOWN_DRIFT_BROWSER` 사유가 대장. 384/400 재활 캠페인의 브라우저판.
- ✅~~verify-eval-ux-193 드리프트~~ **→ 스펙 438에서 수리**(2026-08-04): kind 탭(212)·드로어 탭(252)·SSE networkidle(335) 3겹 현행화, 4건→green. 원 항목: **verify-eval-ux-193.mjs 드리프트**(2026-07-13, 스펙 329 중 stash 확증 — 기존): P1(컬렉션 칩)부터 4건 실패, UI 개편으로 셀렉터 노후 추정. 브라우저 verify 스크립트 전반 노후 점검은 "verify 스위트 격리"(321 대형)와 합류 후보.
...
- [ ] **RAG 인제스트 후속 씨앗**(스펙 334·335 OUT, 2026-07-14): ~~실패 문서 재시도 버튼~~·~~진행률~~ **→ 스펙 435 완료**(2026-07-31, 회고 358: 보존 blob 재인제스트+휘발 진행률 맵, verify 10/10+브라우저 3/3). ~~이벤트 버스 합류~~ **→ 스펙 436 완료**(2026-07-31, 회고 359: 발행 5곳+FE 알림, 검증이 312 이래 잠복한 오류 경로 MissingGreenlet 적중·수리). **잔여**: 영속 알림함(놓친 이벤트 재생).
...
- ✅**#10 피드백→평가 데이터 수확 = 스펙 209 전 Phase 완료**(Phase 1 수집 게이트·1.5 플그 피드백·2 LLM 합성 수확 — spec INDEX 209 참조). DPO는 모델학습 아닌 평가케이스 수확으로 정직 재정의.
