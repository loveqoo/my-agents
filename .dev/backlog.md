...
- [ ] **자원 감사 캠페인 후속(스펙 346~352 OUT, 2026-07-15)**: ①**살아있는 유저 기억·메시지의 상한** — 스코어보드 0은 "분류가 정직하다"는 뜻이지 "무한 증가가 없다"는 뜻이 아니다. 유저 자산의 개수/나이 상한은 **제품 가시 한계라 사용자 승인 사항** → 승인 나면 스펙. ②**RAG 인제스트 메모리**(감사에서 유일하게 미해결로 남긴 축 — ~1.6GB는 **산술 추정이지 실측이 아니다**. 처방 전에 5만 행 인제스트 중 RSS를 **측정**부터. 동시 인제스트 태스크 상한도 없음) ③**배치 트리거의 dry-run 날카로운 모서리**: `POST /admin/batch/{job}/run`은 `dry_run`을 **query**로만 받아, body로 보내면 조용히 무시되고 **진짜 실행**으로 떨어진다(파괴적 잡에서 안전 플래그의 기본값이 false). 클라이언트는 올바르지만 계약이 위험한 쪽으로 기울어 있음.
...
- [ ] **백엔드 리팩토링 캠페인 Tier 2 잔여 2건**(스펙 374, 2026-07-16 중단 — 집중 재개) — codex 4그룹 비판리뷰(활성버그0·전부 구조/드리프트) 위 위험오름차순 3-tier. **완료**: Tier1 DRY단일화(375 MCP캡·376 RAG임베딩검증·377 eval admission)·Tier2 패키지분할(378 schemas→9모듈·379 models→11모듈·380 runtime→rag_runtime). 전부 동작불변(SUITE_OK·e2e39·수치보존) per-spec 커밋·미푸시. **잔여(가장 무거운 둘, 재개 최우선순)**: ①rag.py 1256줄 라우터→`rag/` 패키지(공유 router+collections/reindex/search/documents, 헬퍼가 route그룹 공유라 배선 정교)·②chat_context typed builder(`_load_context` ctx dict→typed DTO, **순수이동 아닌 계약변경**·3파일 관통·모든 ctx[...] 접근처, Tier3 성격이라 그물 강화). 기법=learning 381(verbatim슬라이스+__init__ 재수출+지연import 순환끊기). Tier3(chat→ChatTurnService·eval_runs·jobs파괴잡·pipeline build_graph)는 별도.
