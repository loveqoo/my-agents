...
### 후속 씨앗 (급하지 않음)
- **노드형 오버라이드 세부에 Temperature 재검토**(2026-07-10, learning 152 점검 부산물) — 스펙 287이
  "노드 모델 params 소유"라며 오버라이드 세부에서 Temperature를 뺐지만, 실측(pipeline.py:79)은
  에이전트 temperature(ctx.params)가 **모든 노드에 우선 적용**됨. 거짓 표기는 아니나(숨김일 뿐)
  실효 있는 컨트롤이 오버라이드에서 빠진 상태 — 넣을지(적용 범위 문구와 함께) 유지할지 결정 필요.
