...
- **모델별 비교 시험**(사용자 제안 2026-07-03): 같은 문제집을 지정 모델들로 돌려 나란히 비교 — 에이전트에 적합한 모델 선정용. promptfoo matrix(providers×prompts)·Braintrust experiments가 선례. **단, 구현 전 타 제품 평가 설계 조사 먼저**(사용자 지시 — 조사 진행 중, 결과는 .dev/learning 예정).
