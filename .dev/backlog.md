...
- **도구 다수 시 디스커버 전환(하이브리드 도구 접근)** (2026-07-07, 스펙 202 논의에서 사용자 합의) —
  직접형·플랜형 에이전트의 연결 도구가 임계(예: 10개) 이하면 지금처럼 전부 bind(단순·한 홉), 초과하면
  도구 목록 대신 **브로커 디스커버 창구만** 바인드해 모델이 필요한 도구를 검색→호출(프롬프트 크기
  일정). 에이전트 종류는 불변 — 디스커버는 부품, 조율형(위임·fold·신뢰경계)으로 바뀌는 것 아님(사용자
  문답으로 구분 확정). 선행 참고: discover는 현재 어휘 랭킹(스펙 124), 카탈로그 커지면 벡터 검색 후속.
