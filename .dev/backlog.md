...
- ✅**버전 지정 실행(사용자 방향 삽입)=스펙 242 완료**(2026-07-08, 회고 220 — 내부 버전별 서빙(config 소스 전환)·초안 평가=배포 전 게이트·may_manage 403). **다음=243 플그 버전 선택 UI → D 운영 화면**
- **D. 버전 운영 화면** — 버전별 피드백 수/비율·평가 이력·회귀 결과 집계 뷰
- **C. 수확 케이스 맥락 풍부화** — 수확 시 trace(도구·RAG 흔적) 연결
