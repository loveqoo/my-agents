...
- ✅**스펙 302 = stale verifier 정리 완료**(2026-07-11, 회고 277). 6개(084/100/103/130/131/190) 전수 진단→수선. **190은 stale 아닌 실 프로덕션 회귀**=artifact produce_node config 애노테이션이 langgraph 1.2.5 주입 판정 못 통과(문자열매칭·PEP604 유니언), `RunnableConfig`로 복원(artifact 계열 실사용 크래시 해소). 픽스처 5는 시그니처/명명/컬렉션 드리프트 추종. 6/6 PASS·codex 결함0. **교훈: 실패 verifier "낡았겠지" 방치가 실 회귀 은폐**. ⚠️dev 서버 `--reload`는 packages/agent 미watch→190 반영에 서버 재기동 필요.
