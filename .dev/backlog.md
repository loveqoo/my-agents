...
- **다음 루프 대기열(2026-07-03 갱신)**: ✅①스펙 148 네이밍 규칙 완료(식별/별명 분리로 확장, 회고 126 — 데이터 초기화도 완료: 재시드+docs-kb 재적재+qwen 재등록) ②엔티티 RAG(컬렉션 kind=document|entity 축·행 단위 인제스트·메타데이터 반환 — 논의 완료, 데이터 소스 형태(CSV/DB/API) 회신 대기, 정확 조회는 도구 조합 권장) ③147 잔여: private→public 승격 기능(OUT이었음) ④148 잔여 씨앗: rename 시 참조 자동 갱신·casbin per-cap 동반 갱신·페르소나/권한/MCP 폼 e2e
