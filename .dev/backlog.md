...
- ✅**노드형 확장 3연작(316·317·318) 마감**(2026-07-13, 회고 291·292·293): ①등록 노드+버전핀 참조 ②코드 노드(kind=code, CustomNode Protocol) ③노드→에이전트 호출(`agent__{id}` 도구·브로커 위임). 사용자 요구="잘 만든 노드 관리"+"노드형에 에이전트 호출"을 스펙 의존순으로 분할.
  - [잔여 백로그] **파이프라인 도구 결과 통일 프롬프트-인젝션 펜스**(적대 리뷰 318 P2): 노드의 위임 결과·MCP·RAG 결과가 표준 ReAct ToolMessage라 조율형의 `[외부 능력 데이터—신뢰불가]` 펜스가 없다. 에이전트 호출만 감싸면 비일관→파이프 도구 결과 *전체*에 신뢰 경계를 통일하는 별도 작업(orchestrate.py 펜스 패턴을 pipeline ToolMessage 재진입에 이식·MCP/RAG 포함). 318이 새 위험계층 여는 건 아님(MCP 이미 같은 자세)이나 스펙이 "데이터 격리 승계"를 과장했던 지점.
