...
- **다음 루프 대기열(2026-07-03 회고 시점)**: ①스펙 148 네이밍 규칙(초안 커밋됨·미구현 — 한글 공백 금지 의도 확인 필요) ②엔티티 RAG(컬렉션 kind=document|entity 축·행 단위 인제스트·메타데이터 반환 — 논의 완료, 데이터 소스 형태(CSV/DB/API) 회신 대기, 정확 조회는 도구 조합 권장) ③147 잔여: private→public 승격 기능(OUT이었음)
