...
- [후보 **스펙 301**] dedup 후속 D(census 최대 절감): eval `_execute_*` 배경작업 스캐폴딩 3~4중(next-order·마커 치환·실패 스탬프·`finally: _active_jobs.discard`)·`_execute_suggestion`↔`_execute_generation_append` ~90% 동일. **최대절감~60-100줄이나 `_active_jobs` 락 lifecycle·사용자 노출 description 문구 위험**→async 컨텍스트매니저로 finally-discard 봉인+verify_142/143/195/209 필수. 통합 전 두 함수 라인 diff로 (생성기·name/asserts) 2개 수렴 실측 권장.
