...
- **'능력 부여' UI 개선 (스펙 199 진단 완료, 구현 보류)** — 유저 메뉴 `능력 부여(정책)` 카드가 설계자도
  헷갈릴 만큼 어려움(발견성 실패). 진단=자유입력(이름 타이핑)·`capability:…` 코드 노출·역할/유저 혼재·
  맥락 부족. 개선후보 A(카탈로그 체크박스)·B(문장화)·C(대상 분리)·D(도입 문장). 사용자 결정 "진단만,
  구현 나중" — 착수는 별도 결정 시. 구현 시 RBAC 트리거(grantPolicy write). 상세=docs/spec/199.
