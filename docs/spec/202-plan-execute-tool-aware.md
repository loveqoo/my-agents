...
## 후속 (2026-07-07, 사용자 버그 보고 — "도시락" 세션)
- **인스펙터 도구 카드 중복**: 타임라인의 tools 노드 상세가 `t.mcp` **전부**를 귀속(스펙 192 당시
  가정 "직접 그래프는 tools 노드 1개") → 202의 도구 루프로 tools 노드가 N회 등장하자 **라운드마다
  전체 카드가 중복**(도시락 턴: 3회 호출=카드 9장). 수리: 라운드별 귀속 — ① 호출 result 머리(40자)가
  그 라운드 summary에 포함되면 매칭, ② 매칭 0건이면 순번 폴백. 검증: 동일 시나리오 재현 카드 3=호출
  3(중복 0), 라운드별 내용 일치(유래/어원/벤또) 스샷.
- 진단 중 별도 발견(백로그行): 전송 프롬프트가 실제 모델 입력과 다름(커스텀 impl 자체 구성 프롬프트
  미캡처)·턴 배지가 메시지 인덱스(턴 2,4,6…)·토큰 수치가 추정치로 보임.
