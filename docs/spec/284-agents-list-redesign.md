...
### 후속 3(탭별 검색 요소 정리, 사용자 지시)
- **UI 탭**: 이름(Input)·종류(Select)·A2A 공개만(Checkbox)·상태(Select). 텍스트 검색은 이름 전용
  (283의 종류 텍스트 축은 명시 Select로 대체 — verify-283 갱신).
- **Code 탭**: 이름·A2A(Select: 전체/켬/꺼짐)·상태. **External 탭**: 이름만 — 상태는 필터·컬럼 모두
  제거(사용자: 외부는 우리가 상태 관리하지 않음).
- 탭 전환 시 검색 요소 초기화(잔존 필터의 보이지 않는 작동 방지).
- verify-284(External 부재 단언 포함)·283 갱신 ALL GREEN. e2e 함정: 선택 후 표시값이 바뀌는 Select는
  고정 텍스트 locator가 1회용 — 값 무관 정규식으로 재조회.
