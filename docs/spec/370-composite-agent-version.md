...
ever_opened면 `최대 오픈번호+1`)로 새 스크래치 1행을 쓴다. 개발자 규칙 두 사례가 그대로 성립:
...
- **C5 가드 — 개발자 확정(2026-07-16): 스펙 121 유지.** "특정 버전이 배포됐었다는 이유(롤백)로
  블록을 지우지 못하게 하지 않는다 — 어떤 에이전트도(현재 서빙 config가) 참고하지 않으면 삭제 가능."
  결과 경계: 지운 블록을 못박은 옛 버전으로의 롤백은 완전 재현이 안 될 수 있음(수용) — 죽은 pin은
  resolve_pinned가 None→head 폴백→기존 dangling 경고 degrade로 정직 처리.
