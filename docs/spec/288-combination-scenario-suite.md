...
- **구현 중 실측으로 잡은 계약 4건**(스위트가 첫 실행에서 발견 — 존재 가치 실증).
  **⚠ 1·2는 스펙 289가 계약을 바꿨다**(외부 연동 관점 승격, 2026-07-11) — 아래는 발견 당시 기록,
  현행 계약은 289 참조:
  1. ~~히스토리는 클라이언트 계약~~ → **289 P1**: 재개 세션 + assistant 없는 body면 서버가 DB에서
     이어붙인다(클라 전체 전송도 여전히 유효 — auto 판정). 스위트 run_turns가 두 모드 다 검증.
  2. ~~노드형 풀 파생은 폼의 책임~~ → **289 P2**: 서버가 저장·로드 시 노드 합집합으로 파생 —
     API 직생성도 안전(스위트 픽스처가 풀 미지정으로 실증).
  3. **조율형 위임 게이트**(289 P3서 실측 교정): 위임 대상은 활성 버전 보유(서빙 중, 스펙 256)여야
     후보에 뜬다. lexical 탈락은 `orchestrate`(FirstMatch — 첫 후보 무조건 위임)에는 **없고**
     `orchestrate_ranked`에만 있다(겹침 0 제외 — 영숫자 토큰이 있는데 안 겹칠 때만, 한글-only
     질문은 빈 쿼리 취급 통과). 첫 발견 때 "이름 토큰 필요"라 적은 건 미서빙 탓의 오귀인.
     위임 0건 사유는 이제 trace(plan 노드 delegationNote)가 말한다.
  4. **in-process ASGI는 lifespan 미실행**: HIL 재개용 체크포인터(`init_checkpointer`)와 authz
     (`init_authz` — 비-슈퍼유저 principal 경로)를 러너가 직접 초기화해야 한다.
