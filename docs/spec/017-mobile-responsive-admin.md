...
### 3. 승인 뷰 2단 그리드 미접힘 (1차 패스 누락)
- `ApprovalsView`의 `gridTemplateColumns:'1fr 1fr'`가 모바일에서 안 접혀 카드가 ~190px로
  짜부라지고, 좁은 폭 탓에 "체크포인트…에서 일시정지됨" 텍스트가 한 글자씩 세로로 쪼개짐.
- Overview/Agents와 동일하게 `repeat(auto-fit, minmax(min(100%, 320px), 1fr))`로 교체 →
  모바일 1열, 데스크톱 2단 유지(빈 트랙 collapse). 카드 내용이 많아 min 320px.
