...
## 4-1. 추가 — 톤 멀티셀렉트 (2026-06-24)
> 사용자 요청: "톤을 멀티셀렉트로 미리 정의된 값에서 고르되, 자유 입력도 가능하게."

- 톤 입력 `Input`(자유 텍스트) → antd `Select mode="tags"`로 교체. 프리셋 선택 + 자유 입력(Enter/쉼표) 둘 다 지원.
- 프리셋 10종: 친근함·격식체·간결함·정중함·유머러스·전문적·공감적·단호함·열정적·차분함.
- **저장 형식**: 기존 `tone`(String200) 컬럼 유지, **쉼표 조인 문자열**(`"친근함, 격식체"`)로 저장 → 백엔드/마이그레이션 무변경. 로드 시 split, 목록·상세에서 Tag 여러 개로 렌더.
- 검증: `tsc --noEmit` 통과 / admin E2E에 프리셋 2종 + 자유 1종 선택 → 저장값 `친근함, 격식체, 자유톤` 단언 추가 (22 passed/1 skipped) / codex GATE — P1/P2 0건.
