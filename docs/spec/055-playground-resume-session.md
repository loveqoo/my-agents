...
### 출하 후 보강 — 피커가 시드/레거시 세션에서 깨져 보임 (사용자 보고)
출하 후 사용자가 "Playground UI가 깨진다" 보고. 브라우저 재현 결과 세션 드롭다운에서:
- **시각이 원본 ISO**(`2026-06-28T11:19:59.317619+00:00`)로 렌더돼 줄바꿈·폭 넘침. 백엔드
  `_iso()`가 ISO를 그대로 내리는데 피커가 무가공 출력 → 공용 `admin/src/admin/format.ts`의
  `fmtTime`(오늘=시:분, 올해=M월 D일, 그 외=연.월.일, 파싱불가 문자열은 통과)로 포맷.
  같은 raw-ISO가 SessionsView(목록·상세)에도 있어 함께 적용.
- **메시지 없는 세션이 "(빈 세션)"** 으로 표시 — 시드 세션은 `turns`만 있고 Message 행이
  없어 preview=None인데, 14턴짜리도 "빈 세션"으로 보임. preview 없을 땐 해시를 정직하게 주
  라벨(모노)로, 메타의 해시 중복 제거. preview 있는 실 세션은 기존대로 사람이 읽는 라벨.
- **놓친 이유**: 3-rung 검증이 *방금 만든* 세션만 봤다(오늘 타임스탬프·메시지 있음=preview
  정상). 피커에 함께 뜬 시드 세션(raw ISO·preview 없음)은 E2E 스크린샷에 *찍혀 있었는데도*
  대상 행(marker)만 단언해 형제 행의 결함을 통과시켰다 → learning 057.

### 출하 후 보강 2 — 세션을 골라도 "아무것도 안 나온다" (사용자 보고)
사용자가 "세션을 선택하면 왜 아무것도 안 나오죠? 채팅 히스토리가 나와야 하지 않나요"
보고(인스펙터 정보는 없을 것을 이미 인지). 진단: **`loadSession`은 정상**이었다 —
`getSessionMessages(sid)`가 시드/레거시 세션에 대해 `[]`를 반환(메시지 0행)하고
`setConvos([])` 했다. 문제는 DebugChat이 `messages.length===0`이면 *선택해서 빈* 경우든
*새 대화* 경우든 **똑같은 프롬프트 카드**를 띄워, 골랐는데 빈 화면이 새 대화와 구분되지
않은 것. 데이터 현실: 기본 에이전트(Research Assistant)의 세션은 전부 시드라 `turns`만
있고 Message 0행 → 골라도 표시할 히스토리가 없음(실 히스토리는 Personal Secretary처럼
실제 대화한 세션에만 존재).
- **수정**: `pickedButEmpty = empty && !!currentSessionId && !streaming` 분기를 추가해,
  골랐는데 빈 세션이면 "이 세션에는 불러올 메시지가 없습니다(영속 전/레거시), 입력하면
  이 세션에 이어 쌓입니다"라는 **명시적 빈 상태**를 새-대화 프롬프트 카드와 다르게 띄운다.
- **근본 인사이트**: (1) `Session.turns`는 저장 카운터지 Message 행 수의 집계가 아니다 —
  파생/비정규화 값을 행의 진실로 착각하면 happy-path만 초록. (2) 의미가 다른 두 빈 상태가
  픽셀이 같으면 "선택이 안 먹힌다"로 읽힌다 — 코드는 정상 동작했는데도. → learning 058.
- **검증**: `tests/browser/shot-session-load-055.mjs`(SLOAD_OK) — 시드 세션 선택 시 빈 상태
  노출 + 프롬프트 카드 아님, 피커 행 시각 모두 포맷됨(057 회귀 없음). 메시지 있는 세션의
  히스토리 렌더는 기존 RESUME055_OK(marker 복원=말풍선)가 커버. 스크린샷 눈으로 확인,
  PAGE_ERRORS=[]. `tsc --noEmit` EXIT 0.
