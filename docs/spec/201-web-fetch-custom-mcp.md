...
## 후속 2 (2026-07-07, 사용자 "아무리 해도 발동 안 됨")
근인 두 겹(내 실증과 사용자 경험의 괴리):
1. **배선 부재**: 능력 부여(쓸 권리)≠에이전트 배선(config.mcps). 어떤 에이전트에도 web-fetch가 연결
   안 돼 있었고, 내 실증은 매번 **플그 오버라이드(임시)**로 연결해 성공한 것 — 사용자 기본 경로엔 없었다.
2. **무시되는 배선**: 사용자가 연결해 본 plan-execute-demo는 도구를 아예 안 쓰는 2노드 예제 플로우
   (스펙 085) — 그런데 편집 폼은 커스텀 impl에도 mcps 피커를 노출해 "연결했는데 조용히 무시"가 가능.
처방: research-assistant에 web-fetch **영구 배선**(draft→활성 v5, 편집=초안이라 활성화 필수),
plan-execute-demo 죽은 배선 원복. 사용자 경로(무오버라이드) 재검증 — "백과사전에서 '이순신'"→wiki_page,
"불국사가 언제 지어졌는지"(위키 단어 없음)→wiki_search, mock web_search에 안 뺏김.
남긴 것: "impl이 무시하는 설정을 폼이 경고 없이 수용" 함정은 매니페스트 기반 폼 게이트로 별도(백로그).
